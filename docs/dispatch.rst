...
:file:`src/mesa/glapi/glapitemp.h` contains code very similar to this.
...
considerably larger (e.g., 29,332 lines for :file:`glapi_x86-64.S` versus
1,155 lines for :file:`glapi_x86.S`) and causes simple changes to the
...
:file:`src/mesa/sources`. That gets the file built and linked. The second
step is to add the correct ``#ifdef`` magic to
:file:`src/mesa/glapi/glapi_dispatch.c` to prevent the C version of the
...
:file:`src/mesa/glapi/glprocs.h`. For different reasons on different
...
:file:`src/mesa/glapi/glapi.c` just before :file:`glprocs.h` is included.
