...
:file:`bin/gen_release_notes.py` script. Simply run this script **before**
bumping the version. You'll need to come back to this file once the
tarball is generated to add its SHA256 checksum.

Increment the version contained in the file :file:`VERSION` at Mesa's
op-level, then commit this change and **push the branch** (if you forget
to do this, :file:`release.sh` below will fail).
...
Edit :file:`docs/relnotes/X.Y.Z.rst` to add the SHA256 checksums as
available in the :file:`mesa-X.Y.Z.announce` template. Commit this change.
...
:file:`docs/relnotes.rst` and :file:`docs/release-calendar.csv`. It
...
Parse through the bug reports as listed in the
:file:`docs/relnotes/X.Y.Z.rst` document. If there's outstanding action,
close the bug referencing the commit ID which addresses the bug and
mention the Mesa version that has the fix.
