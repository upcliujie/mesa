...
.. code-block:: console
   ...
   git checkout -b X.Y
   git checkout main
   $EDITOR meson.build  # bump the version number
   git commit -as
...
Increment the version contained in the file ``meson.build`` at Mesa's
top-level, then commit this change and **push the branch** (if you forget to do
