...
The Vulkan runtie code provides a set of base object structs which must be
...
provided `VkObjectType` to assert at runtime that the provided handle is
the correct type of object.  Both cast helpers properly handle `NULL` and
`VK_NULL_HANDLE` as inputs.  The :cpp:any:`VK_FROM_HANDLE()` macro provides
a convenient way to declare a `drv_foo` pointer and initialize it from a
`VkFoo` handle in one smooth motion.
