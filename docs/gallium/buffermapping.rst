...
frames, and the :gl-ext:`ARB_sync` fence has ensured that the GPU has at least started
...
the :gl-ext:`ARB_sync` fence has ensured that the GPU has started frame n-1.
...
In this app, buffer 480 gets used like this every other frame.  The :gl-ext:`ARB_sync`
...
frame.  The :gl-ext:`ARB_sync` fence is used to make sure that the GPU has reached the
...
The :gl-ext:`ARB_sync` fence ensures that the GPU has started frame n-1 before the CPU
