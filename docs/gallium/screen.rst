...
* ``PIPE_CAP_PREFER_BACK_BUFFER_REUSE``: Driver prefers that DRI3 tries to use a difference back buffer each frame, instead of minimizing the number of used back buffers. Only applies to DRI_PRIME.
