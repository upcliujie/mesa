...
def print_job_status(job, new_status=False) -> None:
    ...
    print(
        ...
        + URL_END
        + (f" :: {job.status}" if not new_status else f" has new status: {job.status}")
        + Style.RESET_ALL
    )
...
def monitor_pipeline(
    ...
    statuses = defaultdict(lambda: defaultdict(str))
    target_statuses = defaultdict(lambda: defaultdict(str))
    stress_status = defaultdict(lambda: defaultdict(int))
    target_id = None
        ...
        for job in pipeline.jobs.list(all=True, sort="desc"):
            ...
            if target_jobs_regex.match(job.name):
                ...
                if stress and job.status in ["success", "failed"]:
                    if stress < 0 or sum(stress_status[job.name].values()) < stress:
                        enable_job(project, job, "retry", force_manual)
                        stress_status[job.name][job.status] += 1
                else:
