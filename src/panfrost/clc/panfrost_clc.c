...
static nir_shader *
compile(void *memctx, const uint32_t *spirv, size_t spirv_size, int arch)
{
   const nir_shader_compiler_options *nir_options;
   if (arch >= 9)
      nir_options = &bifrost_nir_options_v9;
   else if (arch >= 6)
      nir_options = &bifrost_nir_options_v6;
   else
      nir_options = &midgard_nir_options;
...
main(int argc, char **argv)
{
   ...
   int ch;
   while ((ch = getopt_long(argc, argv, "h:p:s:i:o:", long_options, NULL)) !=
          -1) {
      switch (ch) {
      case 'h':
   ...
   char pan_arch_opt[100];
   snprintf(pan_arch_opt, 100, "-DPAN_ARCH=%i", arch);
   util_dynarray_append(&clang_args, char *, pan_arch_opt);
   ...
   /* bake mesa version into header */
   fprintf(fp, "#define PANCLC_MESA_GIT_SHA1 \"%s\"\n", MESA_GIT_SHA1);

   /* Compile SPIR-V to NIR */
   nir_shader *nir = compile(mem_ctx, final_spirv.data, final_spirv.size, arch);
