...
TEMPLATE_RS_DEBUG = Template("""\
// This file was generated by class_parser.py, DO NOT EDIT.

pub fn parse_mthd(idx: u32) -> &'static str {
    match idx {
    %for mthd in mthddict:
    %if mthddict[mthd].is_array and mthddict[mthd].array_size == 0:
        <% continue %>
    ...
    %if mthddict[mthd].is_array:
        %for i in range(mthddict[mthd].array_size):
        val if val == crate::${nvcl.lower().replace("nv", "cl")}::mthd::${mthd.lower()}(${i}) => "${nvcl}_${mthd}(${i})",
        %endfor
    % else:
        crate::${nvcl.lower().replace("nv", "cl")}::mthd::${mthd} => "${nvcl}_${mthd}",
    %endif
    %endfor
        _ => "unknown method",
    }
...
pub fn dump_mthd_data(s: &mut impl std::io::Write, idx: u32, data: u32, prefix: &str) -> Result<(), std::io::Error>
{
    match idx {
    %for mthd in mthddict:
    %if mthddict[mthd].is_array and mthddict[mthd].array_size == 0:
    ...
    %if mthddict[mthd].is_array:
        <%
            guard = ""
            for i in range(mthddict[mthd].array_size):
                guard += f"val == crate::{nvcl.lower().replace("nv", "cl")}::mthd::{mthd.lower()}({i})  "
                if i != len(range(mthddict[mthd].array_size)) - 1:
                    guard += "|| "
        %>
        val if ${guard} => {
    % else:
        crate::${nvcl.lower().replace("nv", "cl")}::mthd::${mthd} => {
    %endif
    %for field_name in mthddict[mthd].field_name_start:
        <%
            field_start = int(mthddict[mthd].field_name_start[field_name])
        ...
        %if field_width == 32:
            let parsed = data;
        %else:
            let parsed = (data >> ${field_start}) & ((1 << ${field_width}) - 1);
        %endif
            write!(s, "{}.${field_name} = ", prefix)?;
        %if len(mthddict[mthd].field_defs[field_name]):
            match parsed {
            %for d in mthddict[mthd].field_defs[field_name]:
                %if "TYPE_" in field_name or "OVERRIDE_" in field_name:
                    <% field_name = field_name.rstrip('_') %>
                %endif
                crate::${nvcl.lower().replace("nv", "cl")}::mthd::${mthd}_${field_name}_${d} => {
                    write!(s, "${d}${bs}n")?;
                }
            %endfor
                _ => {
                    write!(s, "0x{:x}${bs}n", parsed)?;
                }
            }
        %else:
            %if mthddict[mthd].is_float:
            write!(s, "{}f (0x{:x})${bs}n", parsed as f32, parsed)?;
            %else:
            write!(s, "(0x{:x})${bs}n", parsed)?;
            %endif
        %endif
    ...
    %endfor
        _ => {
            write!(s, "{}.VALUE = 0x{:x}${bs}n", prefix, data)?;
        }
    }
...
def main():
    ...
    parser.add_argument('--out-rs-debug', required=False,
                        help='Output Rust file for debug information.')
    ...
    try:
        ...
        if args.out_rs_debug is not None:
            with open(args.out_rs_debug, 'w', encoding='utf-8') as f:
                f.write(TEMPLATE_RS_DEBUG.render(**environment))
