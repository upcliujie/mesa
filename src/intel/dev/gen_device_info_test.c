...
#include "pci_ids/iris_pci_ids.h"
   ...
   for (uint32_t i = 0; i < ARRAY_SIZE(chipsets); i++) {
      ...
      assert(gen_device_info_subslice_total(&devinfo) <=
      ...
      for (uint32_t i = 0; i < ARRAY_SIZE(devinfo.subslice_masks); i++)
         total_subslices += __builtin_popcount(devinfo.subslice_masks[i]);
      assert(total_subslices <=
             (devinfo.max_slices * devinfo.max_subslices_per_slice));

      assert(gen_device_info_eu_total(&devinfo) > 0);
      assert(gen_device_info_subslice_total(&devinfo) > 0);

      total_subslices = 0;
      for (uint32_t s = 0; s < devinfo.max_slices; s++)
         for (uint32_t ss = 0; ss < devinfo.max_subslices_per_slice; ss++)
            total_subslices += gen_device_info_subslice_available(&devinfo, s, ss);
      assert(total_subslices == gen_device_info_subslice_total(&devinfo));

      uint32_t total_eus = 0;
      for (uint32_t s = 0; s < devinfo.max_slices; s++)
         for (uint32_t ss = 0; ss < devinfo.max_subslices_per_slice; ss++)
            for (uint32_t eu = 0; eu < devinfo.max_eu_per_subslice; eu++)
               total_eus += gen_device_info_eu_available(&devinfo, s, ss, eu);
      assert(total_eus == gen_device_info_eu_total(&devinfo));
   }
