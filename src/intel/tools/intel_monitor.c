...
print_help(const char *progname, FILE *file)
{
   fprintf(file,
      ...
      "      -t, --sample_time     period of sampling, in microseconds. DEFAULT=100\n"
      "\n"
      "Eu stall sample usage:\n"
      "   0. Run `sudo sysctl dev.xe.observation_paranoid=0`\n"
      "   1. Launch gfx app with VK_ENABLE_PIPELINE_CACHE=1 INTEL_DEBUG=shaders-lineno\n"
...
main(int argc, char *argv[])
{
   ...
   const struct option intel_monitor_opts[] = {
      ...
      { "sample_time",  optional_argument, NULL, 't' },
      { NULL,           0,                 NULL, 0 }
   };
   ...
   /* Parse arguments */
   while ((c = getopt_long(argc, argv, "heft", intel_monitor_opts, &i)) != -1) {
      switch (c) {
      case 'h':
   ...
   if (sample_mode == INTEL_MONITOR_MODE_EUSTALL) {
      ...
      cfg = eustall_setup(drm_fd, &devinfo, sample_period_us * 1000);

      do_sample = (bool (*)(void*))eustall_sample;
      do_dump_results = (void (*)(void*, FILE*))eustall_dump_results;
      do_close = (void (*)(void*))eustall_close;
   } else {
      fprintf(stderr,
