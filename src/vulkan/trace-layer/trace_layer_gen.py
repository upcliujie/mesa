...
PFN_vkVoidFunction
traceGetLayerProcAddr(const char *pName, bool device_only);
...
% if e.name == 'GetInstanceProcAddr':
   const auto addr = traceGetLayerProcAddr(pName, false);
   if (addr)
      return addr;
% elif e.name == 'GetDeviceProcAddr':
   const auto addr = traceGetLayerProcAddr(pName, true);
   if (addr)
      return addr;
% endif
</%def>
...
<%def name="dispatch(e)">\\
% if e.name in ['CreateInstance', 'EnumeratePhysicalDevices', 'EnumeratePhysicalDeviceGroups', 'EnumeratePhysicalDeviceGroupsKHR']:
   const auto ret = TraceInstance::${e.name}(${e.call_params()});
% elif e.name in ['CreateDevice', 'CreateCommandPool', 'AllocateCommandBuffers']:
   const auto ret = TraceDevice::${e.name}(${e.call_params()});
...
% elif e.name in ['QueueSubmit']:
   const auto ret = TraceQueue::${e.name}(${e.call_params()});
