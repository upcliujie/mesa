...
udev_event_listener_thread(void *data)
{
   ...
   /* We need to have fd for matching. */
   struct stat wsi_st;
   if (fstat(wsi->syncobj_fd, &wsi_st) != 0)
      goto fail;

   unsigned wsi_device_major = major(wsi_st.st_rdev);
   ...
   for (;;) {
      ...
      if (ret > 0) {
         if (fds[0].revents & POLLIN) {
            ...
            struct stat udev_device_st;
            if (stat(udev_device_get_devnode(dev), &udev_device_st) != 0)
               continue;

            /* Ignore event if it is not a hotplug event or device does not
             * match with wsi_device.
             */
            if (!atoi(udev_device_get_property_value(dev, "HOTPLUG")) ||
                major(udev_device_st.st_rdev != wsi_device_major))
               continue;
