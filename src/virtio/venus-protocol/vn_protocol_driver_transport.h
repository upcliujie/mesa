...
/* struct VkVenusExperimentalFeatures100000MESA */

static inline size_t
vn_sizeof_VkVenusExperimentalFeatures100000MESA(const VkVenusExperimentalFeatures100000MESA *val)
{
    size_t size = 0;
    size += vn_sizeof_VkBool32(&val->memoryResourceAllocationSize);
    size += vn_sizeof_array_size(63);
    size += vn_sizeof_VkBool32_array(val->zerosReserved, 63);
    return size;
}

static inline void
vn_decode_VkVenusExperimentalFeatures100000MESA(struct vn_cs_decoder *dec, VkVenusExperimentalFeatures100000MESA *val)
{
    vn_decode_VkBool32(dec, &val->memoryResourceAllocationSize);
    {
        const size_t array_size = vn_decode_array_size(dec, 63);
        vn_decode_VkBool32_array(dec, val->zerosReserved, array_size);
    }
}

static inline size_t
vn_sizeof_VkVenusExperimentalFeatures100000MESA_partial(const VkVenusExperimentalFeatures100000MESA *val)
{
    size_t size = 0;
    /* skip val->memoryResourceAllocationSize */
    /* skip val->zerosReserved */
    return size;
}

static inline void
vn_encode_VkVenusExperimentalFeatures100000MESA_partial(struct vn_cs_encoder *enc, const VkVenusExperimentalFeatures100000MESA *val)
{
    /* skip val->memoryResourceAllocationSize */
    /* skip val->zerosReserved */
}
...
static inline size_t vn_sizeof_vkGetVenusExperimentalFeatures100000MESA(VkVenusExperimentalFeatures100000MESA* pFeatures)
{
    const VkCommandTypeEXT cmd_type = VK_COMMAND_TYPE_vkGetVenusExperimentalFeatures100000MESA_EXT;
    const VkFlags cmd_flags = 0;
    size_t cmd_size = vn_sizeof_VkCommandTypeEXT(&cmd_type) + vn_sizeof_VkFlags(&cmd_flags);

    cmd_size += vn_sizeof_simple_pointer(pFeatures);
    if (pFeatures)
        cmd_size += vn_sizeof_VkVenusExperimentalFeatures100000MESA_partial(pFeatures);

    return cmd_size;
}

static inline void vn_encode_vkGetVenusExperimentalFeatures100000MESA(struct vn_cs_encoder *enc, VkCommandFlagsEXT cmd_flags, VkVenusExperimentalFeatures100000MESA* pFeatures)
{
    const VkCommandTypeEXT cmd_type = VK_COMMAND_TYPE_vkGetVenusExperimentalFeatures100000MESA_EXT;
    ...
    if (vn_encode_simple_pointer(enc, pFeatures))
        vn_encode_VkVenusExperimentalFeatures100000MESA_partial(enc, pFeatures);
}

static inline size_t vn_sizeof_vkGetVenusExperimentalFeatures100000MESA_reply(VkVenusExperimentalFeatures100000MESA* pFeatures)
{
    const VkCommandTypeEXT cmd_type = VK_COMMAND_TYPE_vkGetVenusExperimentalFeatures100000MESA_EXT;
    size_t cmd_size = vn_sizeof_VkCommandTypeEXT(&cmd_type);
    ...
    cmd_size += vn_sizeof_simple_pointer(pFeatures);
    if (pFeatures)
        cmd_size += vn_sizeof_VkVenusExperimentalFeatures100000MESA(pFeatures);

    return cmd_size;
}

static inline void vn_decode_vkGetVenusExperimentalFeatures100000MESA_reply(struct vn_cs_decoder *dec, VkVenusExperimentalFeatures100000MESA* pFeatures)
{
    VkCommandTypeEXT command_type;
    vn_decode_VkCommandTypeEXT(dec, &command_type);
    assert(command_type == VK_COMMAND_TYPE_vkGetVenusExperimentalFeatures100000MESA_EXT);

    if (vn_decode_simple_pointer(dec)) {
        vn_decode_VkVenusExperimentalFeatures100000MESA(dec, pFeatures);
    } else {
        pFeatures = NULL;
    }
}
...
static inline void vn_submit_vkGetVenusExperimentalFeatures100000MESA(struct vn_instance *vn_instance, VkCommandFlagsEXT cmd_flags, VkVenusExperimentalFeatures100000MESA* pFeatures, struct vn_instance_submit_command *submit)
{
    uint8_t local_cmd_data[VN_SUBMIT_LOCAL_CMD_SIZE];
    void *cmd_data = local_cmd_data;
    size_t cmd_size = vn_sizeof_vkGetVenusExperimentalFeatures100000MESA(pFeatures);
    if (cmd_size > sizeof(local_cmd_data)) {
        cmd_data = malloc(cmd_size);
        if (!cmd_data)
            cmd_size = 0;
    }
    const size_t reply_size = cmd_flags & VK_COMMAND_GENERATE_REPLY_BIT_EXT ? vn_sizeof_vkGetVenusExperimentalFeatures100000MESA_reply(pFeatures) : 0;

    struct vn_cs_encoder *enc = vn_instance_submit_command_init(vn_instance, submit, cmd_data, cmd_size, reply_size);
    if (cmd_size) {
        vn_encode_vkGetVenusExperimentalFeatures100000MESA(enc, cmd_flags, pFeatures);
        vn_instance_submit_command(vn_instance, submit);
        if (cmd_data != local_cmd_data)
            free(cmd_data);
    }
...
static inline void vn_call_vkGetVenusExperimentalFeatures100000MESA(struct vn_instance *vn_instance, VkVenusExperimentalFeatures100000MESA* pFeatures)
{
    struct vn_instance_submit_command submit;
    vn_submit_vkGetVenusExperimentalFeatures100000MESA(vn_instance, VK_COMMAND_GENERATE_REPLY_BIT_EXT, pFeatures, &submit);
    struct vn_cs_decoder *dec = vn_instance_get_command_reply(vn_instance, &submit);
    if (dec) {
        vn_decode_vkGetVenusExperimentalFeatures100000MESA_reply(dec, pFeatures);
        vn_instance_free_command_reply(vn_instance, &submit);
    }
}

static inline void vn_async_vkGetVenusExperimentalFeatures100000MESA(struct vn_instance *vn_instance, VkVenusExperimentalFeatures100000MESA* pFeatures)
{
    struct vn_instance_submit_command submit;
    vn_submit_vkGetVenusExperimentalFeatures100000MESA(vn_instance, 0, pFeatures, &submit);
}
