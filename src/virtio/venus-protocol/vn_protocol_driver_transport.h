...
{
    size_t size = 0;
    size += vn_sizeof_uint32_t(&val->memoryResourceAllocationSize);
    size += vn_sizeof_array_size(63);
    size += vn_sizeof_uint32_t_array(val->zerosReserved, 63);
    return size;
}
...
{
    vn_decode_uint32_t(dec, &val->memoryResourceAllocationSize);
    {
        const size_t array_size = vn_decode_array_size(dec, 63);
        vn_decode_uint32_t_array(dec, val->zerosReserved, array_size);
    }
}
