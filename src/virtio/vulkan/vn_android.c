...
struct vn_android_image_create_info {
...
static VkResult
vn_android_add_image_modifier_info(
   struct vn_device *dev,
   const VkImageCreateInfo *create_info,
   const native_handle_t *handle,
   struct vn_android_image_create_info *out_info)
{
   ...
   result = vn_android_get_modifier_properties(
      dev, create_info->format, buf_props.modifier, &mod_props);
   if (result != VK_SUCCESS)
      return result;

   memset(out_info->layouts, 0, sizeof(out_info->layouts));
   for (uint32_t i = 0; i < mod_props.drmFormatModifierPlaneCount; i++) {
      out_info->layouts[i].offset = buf_props.offset[i];
      out_info->layouts[i].rowPitch = buf_props.stride[i];
   }
   out_info->modifier = (VkImageDrmFormatModifierExplicitCreateInfoEXT){
      .sType =
      ...
      .pPlaneLayouts = out_info->layouts,
   };
   out_info->external = (VkExternalMemoryImageCreateInfo){
      .sType = VK_STRUCTURE_TYPE_EXTERNAL_MEMORY_IMAGE_CREATE_INFO,
      .pNext = &out_info->modifier,
      .handleTypes = VK_EXTERNAL_MEMORY_HANDLE_TYPE_DMA_BUF_BIT_EXT,
   };
   out_info->create = *create_info;
   out_info->create.pNext = &out_info->external;
   out_info->create.tiling = VK_IMAGE_TILING_DRM_FORMAT_MODIFIER_EXT;
                          ...
                          struct vn_image **out_img)
{
   ...
   int dma_buf_fd = -1;
   int dup_fd = -1;
   struct vn_android_image_create_info local_info;
   ...
   result = vn_android_add_image_modifier_info(dev, create_info,
                                               anb_info->handle, &local_info);
   if (result != VK_SUCCESS)
      goto fail;

   /* encoder will strip the Android specific pNext structs */
   result = vn_image_create(dev, &local_info.create, alloc, &img);
   if (result != VK_SUCCESS)
      goto fail;
                             ...
                             struct AHardwareBuffer *ahb)
{
   ...
   if (dedicated_info && dedicated_info->image != VK_NULL_HANDLE) {
      ...
      result = vn_android_add_image_modifier_info(
         dev, &img->deferred_info->create, handle, &local_info);
      if (result != VK_SUCCESS)
         return result;

      result = vn_image_init_deferred(dev, &local_info.create, img);
      if (result != VK_SUCCESS)
         return result;
