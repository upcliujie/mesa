...
vn_cs_renderer_protocol_info_init(struct vn_instance *instance)
{
   ...
   if (VN_DEBUG(INIT)) {
      vn_log(instance, "renderer protocol supports below extensions:");
      for (uint32_t i = 0; i < _vn_info_extension_count; i++) {
         if (vn_cs_renderer_protocol_has_extension(_vn_info_extensions[i].number))
            vn_log(instance, "\t%s", _vn_info_extensions[i].name);
      }
   }
}
