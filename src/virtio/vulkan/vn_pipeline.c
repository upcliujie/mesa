   ...
   VkGraphicsPipelineCreateInfo **out)
{
   ...
   /* Does any pCreateInfo need a fix? */
   bool any_fix = false;

   struct vn_graphics_pipeline_create_info_fix *fix =
      vk_zalloc(alloc, sizeof(*fix) * create_info_count, VN_DEFAULT_ALIGN,
   ...
   for (uint32_t i = 0; i < create_info_count; i++) {
      ...
      if (info->pRasterizationState->rasterizerDiscardEnable == VK_TRUE &&
         ...
         fix[i].ignore_raster_dedicated_states = true;
         any_fix = true;
      }
   }
