...
vn_physical_device_init_properties(struct vn_physical_device *physical_dev)
{
   ...
   if (physical_dev->renderer_extensions.EXT_line_rasterization) {
      physical_dev->line_rasterization_properties.sType =
         VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_LINE_RASTERIZATION_PROPERTIES_EXT;
      physical_dev->line_rasterization_properties.pNext =
         physical_dev->properties.pNext;
      physical_dev->properties.pNext =
         &physical_dev->line_rasterization_properties;
   }

   if (physical_dev->renderer_extensions.EXT_provoking_vertex) {
      physical_dev->provoking_vertex_properties.sType =
         VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_PROVOKING_VERTEX_PROPERTIES_EXT;
      physical_dev->provoking_vertex_properties.pNext =
         physical_dev->properties.pNext;
      physical_dev->properties.pNext =
