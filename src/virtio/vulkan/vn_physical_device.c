                           ...
                           uint32_t *out_count)
{
   ...
   for (uint32_t i = 0; i < count; i++) {
      ...
      if (result != VK_SUCCESS) {
         for (struct vn_physical_device *tmp = physical_devs;
              tmp < physical_dev; tmp++)
            vn_physical_device_base_fini(&tmp->base);
         goto out;
      }
   ...
   for (uint32_t i = 0; i < count; i++) {
      ...
      if (result != VK_SUCCESS) {
         for (uint32_t j = i; j < count; j++) {
            struct vn_physical_device *tmp = &physical_devs[j];
            vn_physical_device_base_fini(&tmp->base);
         }
         count = i;
         goto out;
...
out:
   if (physical_devs && (result != VK_SUCCESS || !count)) {
      for (uint32_t i = 0; i < count; i++)
