...
VkResult
vn_device_memory_import_fd(struct vn_device *dev,
                           struct vn_device_memory *mem,
                           ...
                           const VkImportMemoryFdInfoKHR *fd_info)
{
   ...
   const VkExportMemoryAllocateInfo *export_info =
      vk_find_struct_const(mem_info->pNext, EXPORT_MEMORY_ALLOCATE_INFO);
   const VkPhysicalDeviceMemoryProperties *mem_props =
      &dev->physical_device->memory_properties.memoryProperties;
   const VkMemoryType *mem_type =
      &mem_props->memoryTypes[mem_info->memoryTypeIndex];
   ...
   result = vn_renderer_bo_create_from_dmabuf(
      dev->renderer, mem_info->allocationSize, fd_info->fd,
