 ...
 *
 * VK_IMAGE_LAYOUT_PRESENT_SRC_KHR cannot be used for other purposes but for
 * presentation.  That allows us to treat the layout internally as
 *
 *  - VK_IMAGE_LAYOUT_GENERAL
 *  - VK_QUEUE_FAMILY_FOREIGN_EXT has the ownership
 *
 * The spec also says
 *
 *    If an application does not need the contents of a resource to remain
 *    valid when transferring from one queue family to another, then the
 *    ownership transfer should be skipped.
 *
 * That allows us to use VK_IMAGE_LAYOUT_PRESENT_SRC_KHR as the only signal to
 * trigger queue family ownership transfers.  When VK_IMAGE_LAYOUT_UNDEFINED
 * is specified, we would safely skip the ownership transfer and leave the
 * contents undefined.
 *
 ...
 * other queue family is.  It is easier when the barrier does not define a
 * queue family ownership transfer (i.e., srcQueueFamilyIndex equals to
 ...
 * we are only interested in ownership release or acquire respectively and
 * should avoid double releases/acquires.
 *
                                ...
                                VkImageMemoryBarrier *img_barrier)
{
   ...
   *img_barrier = *src_barrier;

   /* no fix needed */
   if (img_barrier->oldLayout != VK_IMAGE_LAYOUT_PRESENT_SRC_KHR &&
   ...
   if (img_barrier->oldLayout == img_barrier->newLayout) {
      ...
      if (img_barrier->newLayout == VK_IMAGE_LAYOUT_PRESENT_SRC_KHR)
         img_barrier->newLayout = VK_IMAGE_LAYOUT_GENERAL;
      return;
   }

   /* XXX when prime blitting, we should skip ownership transfers */
   ...
   if (img_barrier->oldLayout == VK_IMAGE_LAYOUT_PRESENT_SRC_KHR) {
      ...
      const uint32_t dst_qfi = img_barrier->dstQueueFamilyIndex;
      if (img->sharing_mode == VK_SHARING_MODE_CONCURRENT) {
         img_barrier->srcQueueFamilyIndex = VK_QUEUE_FAMILY_FOREIGN_EXT;
         img_barrier->dstQueueFamilyIndex = VK_QUEUE_FAMILY_IGNORED;
      } else if (dst_qfi == img_barrier->srcQueueFamilyIndex ||
                 dst_qfi == cmd->queue_family_index) {
         img_barrier->srcQueueFamilyIndex = VK_QUEUE_FAMILY_FOREIGN_EXT;
         img_barrier->dstQueueFamilyIndex = cmd->queue_family_index;
      } else {
         /* no trasfer nor transition */
      ...
      const uint32_t src_qfi = img_barrier->srcQueueFamilyIndex;
      if (img->sharing_mode == VK_SHARING_MODE_CONCURRENT) {
         img_barrier->srcQueueFamilyIndex = VK_QUEUE_FAMILY_IGNORED;
         img_barrier->dstQueueFamilyIndex = VK_QUEUE_FAMILY_FOREIGN_EXT;
      } else if (src_qfi == img_barrier->dstQueueFamilyIndex ||
                 src_qfi == cmd->queue_family_index) {
         img_barrier->srcQueueFamilyIndex = cmd->queue_family_index;
         img_barrier->dstQueueFamilyIndex = VK_QUEUE_FAMILY_FOREIGN_EXT;
      } else {
         /* no trasfer nor transition */
         img_barrier->srcQueueFamilyIndex = VK_QUEUE_FAMILY_IGNORED;
...
vn_cmd_wait_events_fix_image_memory_barriers(
   ...
   uint32_t count,
   const VkImageMemoryBarrier **out_acquire_barriers,
   uint32_t *out_acquire_count,
   const VkImageMemoryBarrier **out_release_barriers,
   uint32_t *out_release_count)
{
   ...
   *out_release_count = 0;
   ...
   for (uint32_t i = 0; i < count; i++) {
      const VkImageMemoryBarrier *img_barrier = &img_barriers[i];
      if (img_barrier->srcQueueFamilyIndex ==
          img_barrier->dstQueueFamilyIndex)
         continue;

      if (img_barrier->srcQueueFamilyIndex == VK_QUEUE_FAMILY_FOREIGN_EXT)
         acquire_count++;
      else
         release_count++;
   }
   ...
   VkImageMemoryBarrier *acquire_barriers = img_barriers + count;
   VkImageMemoryBarrier *release_barriers = acquire_barriers + acquire_count;
   acquire_count = 0;
   release_count = 0;
   for (uint32_t i = 0; i < count; i++) {
      VkImageMemoryBarrier *img_barrier = &img_barriers[i];
      if (img_barrier->srcQueueFamilyIndex ==
          img_barrier->dstQueueFamilyIndex)
         continue;
      ...
      /* TODO is this right? */
      if (img_barrier->srcQueueFamilyIndex == VK_QUEUE_FAMILY_FOREIGN_EXT) {
         acquire_barriers[acquire_count++] = *img_barrier;
         img_barrier->oldLayout = img_barrier->newLayout;
      } else {
         release_barriers[release_count++] = *img_barrier;
         img_barrier->newLayout = img_barrier->oldLayout;
      }
      img_barrier->srcQueueFamilyIndex = VK_QUEUE_FAMILY_IGNORED;
      img_barrier->dstQueueFamilyIndex = VK_QUEUE_FAMILY_IGNORED;
   ...
   *out_acquire_barriers = acquire_barriers;
   *out_acquire_count = acquire_count;
   *out_release_barriers = release_barriers;
   *out_release_count = release_count;
...
static void
vn_attachment_to_image_memory_barrier(
   ...
   VkImageMemoryBarrier *img_barrier)
{
   *img_barrier = (VkImageMemoryBarrier)
   ...
   if (att->acquire) {
      img_barrier->srcAccessMask = att->src_access_mask;
      img_barrier->dstAccessMask = att->dst_access_mask;
   ...
   uint32_t count)
{
   ...
   for (uint32_t i = 0; i < count; i++) {
      ...
      vn_attachment_to_image_memory_barrier(images[i], &atts[i],
                                            &img_barriers[i]);
                         ...
                         const VkRenderPassBeginInfo *begin_info)
{
   ...
   if (!pass->present_src_count || !begin_info)
      return;
                        ...
                        const VkBufferImageCopy *pRegions)
{
   ...
   /* XXX the image is owned by the foreign queue family */
                 ...
                 const VkImageMemoryBarrier *pImageMemoryBarriers)
{
   ...
   pImageMemoryBarriers = vn_cmd_wait_events_fix_image_memory_barriers(
      cmd, pImageMemoryBarriers, imageMemoryBarrierCount, &acquire_barriers,
      &acquire_count, &release_barriers, &release_count);

   if (acquire_count) {
      vn_cmd_encode_memory_barriers(cmd, srcStageMask, dstStageMask, 0, NULL,
                                    acquire_count, acquire_barriers);
   }
   ...
   if (release_count) {
      vn_cmd_encode_memory_barriers(cmd, srcStageMask, dstStageMask, 0, NULL,
                                    release_count, release_barriers);
   }
}
