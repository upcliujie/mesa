...
vn_instance_init_renderer(struct vn_instance *instance)
{
   ...
   if (renderer_info->vk_ext_command_serialization_spec_version > version) {
      renderer_info->vk_ext_command_serialization_spec_version = version;
   }

   version = vn_info_extension_spec_version("VK_MESA_venus_protocol");
   if (renderer_info->vk_mesa_venus_protocol_spec_version > version) {
      renderer_info->vk_mesa_venus_protocol_spec_version = version;
   }
