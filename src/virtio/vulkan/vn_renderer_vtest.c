...
vtest_bo_map(struct vn_renderer *renderer, struct vn_renderer_bo *_bo)
{
   ...
   if (!bo->base.ptr && mappable) {
       ...
       * lie.
       */
      void *ptr = mmap(NULL, bo->base.size, PROT_READ | PROT_WRITE,
                       MAP_SHARED, bo->res_fd, 0);
      if (ptr == MAP_FAILED) {
         vn_log(vtest->instance,
                "failed to mmap %d of size %" PRIu64 " rw: %s", bo->res_fd,
                bo->base.size, strerror(errno));
      } else {
         bo->base.ptr = ptr;
...
static VkResult
vtest_bo_create_from_memory(struct vn_renderer *renderer,
                            VkDeviceSize size,
                            vn_object_id mem_id,
                    ...
                    struct vn_renderer_shmem *_shmem)
{
   ...
   munmap(shmem->base.ptr, shmem->base.size);
...
vtest_init(struct vtest *vtest)
{
   ...
   vtest->base.bo_ops.create_from_memory = vtest_bo_create_from_memory;
