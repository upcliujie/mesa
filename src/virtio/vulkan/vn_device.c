...
{
   VkResult result =
      vn_renderer_create(instance, &instance->allocator, &instance->renderer);
   ...
   if (instance->renderer_info.wire_format_version != version) {
      if (VN_DEBUG(INIT)) {
         vn_log(NULL, "wire format version %d != %d",
   ...
   if (VN_DEBUG(INIT)) {
      vn_log(NULL, "connected to renderer");
      vn_log(NULL, "wire format version %d",
             instance->renderer_info.wire_format_version);
      vn_log(NULL, "vk xml version %d.%d.%d",
      ...
      vn_log(
         NULL, "VK_EXT_command_serialization spec version %d",
         instance->renderer_info.vk_ext_command_serialization_spec_version);
      vn_log(NULL, "VK_MESA_venus_protocol spec version %d",
             instance->renderer_info.vk_mesa_venus_protocol_spec_version);
   }

   vn_cs_init(&instance->cs, &instance->allocator,
   ...
   if (result != VK_SUCCESS) {
      if (VN_DEBUG(INIT))
         vn_log(NULL, "failed to enumerate renderer instance version");
      return result;
   }

   if (renderer_version < VN_MIN_RENDERER_VERSION) {
      if (VN_DEBUG(INIT)) {
         vn_log(NULL, "unsupported renderer instance version %d.%d",
   ...
   if (VN_DEBUG(INIT)) {
      vn_log(NULL, "vk instance version %d.%d.%d",
             VK_VERSION_MAJOR(instance->renderer_version),
             VK_VERSION_MINOR(instance->renderer_version),
...
vn_instance_grow_cs_reply_locked(struct vn_instance *instance, size_t size)
{
   ...
   VkResult result =
      vn_renderer_bo_create_shm(instance->renderer, bo_size, alloc,
   ...
   if (vn_cs_reserve_out(&instance->cs, cmd_size))
      vn_encode_vkSetReplyCommandStreamMESA(&instance->cs, 0, &stream);
...
vn_physical_device_init_version(struct vn_physical_device *physical_dev)
{
   ...
   if (props.apiVersion < VN_MIN_RENDERER_VERSION) {
      if (VN_DEBUG(INIT)) {
         vn_log(NULL, "unsupported renderer device version %d.%d",
      ...
      if (result == VK_SUCCESS) {
         ...
         for (uint32_t j = 0; j < physical_dev->queue_family_count; j++) {
            ...
            if (sync_queue_base + props->queueCount >
                instance->renderer_info.sync_queue_count) {
              ...
              const VkAllocationCallbacks *alloc)
{
   ...
   VkResult result =
      vn_renderer_sync_create(dev->instance->renderer, alloc,
                              VK_SYSTEM_ALLOCATION_SCOPE_DEVICE, &sync);
   if (result != VK_SUCCESS)
      return result;

   result = vn_renderer_sync_init_fence(sync, false, 0);
   if (result != VK_SUCCESS) {
      vn_renderer_sync_destroy(sync, alloc);
      return result;
   }
                ...
                VkDevice *pDevice)
{
   ...
   if (queue_count < dev->queue_count) {
      for (uint32_t i = 0; i < queue_count; i++) {
         ...
         vn_renderer_sync_destroy(queue->sync, alloc);
      }
...
vn_queue_submission_alloc_storage(struct vn_queue_submission *submit)
{
   ...
   if (submit->sync_count) {
      ...
      sync_points_offset = alloc_size;
      alloc_size += sizeof(*submit->temp.sync_points) * submit->sync_count;
   ...
   submit->temp.sync_points = submit->temp.storage + sync_points_offset;
...
vn_queue_submission_setup_batches(struct vn_queue_submission *submit)
{
   ...
   uint64_t *sync_points = submit->temp.sync_points;
   switch (submit->batch_type) {
   case VK_STRUCTURE_TYPE_SUBMIT_INFO:
      for (uint32_t i = 0; i < submit->batch_count; i++) {
         const VkSubmitInfo *batch = &submit->submit_batches[i];
         ...
         if (submit->signal_semaphore_count >
            ...
            for (uint32_t j = 0; j < batch->signalSemaphoreCount; j++) {
               ...
               *syncs++ = payload->sync;
               *sync_points++ = sem->type == VK_SEMAPHORE_TYPE_TIMELINE
   ...
   assert(submit->temp.sync_points + submit->sync_count == sync_points);
}
               ...
               VkFence fence)
{
   ...
   if (submit.signal_timeline_count) {
      ...
      for (uint32_t i = 0; i < submit.batch_count - 1; i++) {
         ...
         vn_async_flush(dev->instance);

         renderer->submit(
            renderer, &(const struct vn_renderer_submit){
                         .sync_queue_index = queue->sync_queue_index,
                         .sync_queue_id = queue->base.id,
                         .syncs = &submit.temp.syncs[sync_base],
                         .sync_points = &submit.temp.sync_points[sync_base],
      ...
      if (submit.sync_count) {
         ...
         renderer->submit(renderer,
                          &(const struct vn_renderer_submit){
                             ...
                             .sync_points = submit.temp.sync_points,
...
vn_QueueWaitIdle(VkQueue _queue)
{
   ...
   const uint64_t point = ++queue->sync_point;
   renderer->submit(renderer, &(const struct vn_renderer_submit){
                                 .sync_queue_index = queue->sync_queue_index,
                                 .sync_queue_id = queue->base.id,
                                 .syncs = &queue->sync,
                                 .sync_points = &point,
   ...
   VkResult result =
      vn_renderer_wait(renderer, &queue->sync, &point, 1, true, UINT64_MAX);
                       ...
                       const VkAllocationCallbacks *alloc)
{
   ...
   result = vn_renderer_sync_init_fence(perm_sync, signaled, 0);
   if (result != VK_SUCCESS) {
   ...
   result =
      vn_renderer_sync_create(dev->instance->renderer, alloc,
                              VK_SYSTEM_ALLOCATION_SCOPE_OBJECT, &temp_sync);
   if (result != VK_SUCCESS) {
      vn_renderer_sync_destroy(perm_sync, alloc);
      return result;
   }
                 ...
                 uint64_t timeout)
{
   ...
   if (wait_count && (waitAll || !signaled_count)) {
      result = vn_renderer_wait(dev->instance->renderer, syncs, NULL,
                                wait_count, !waitAll, timeout);
   }
                           ...
                           const VkAllocationCallbacks *alloc)
{
   ...
   if (sem->type == VK_SEMAPHORE_TYPE_TIMELINE) {
      result = vn_renderer_sync_init_semaphore(perm_sync, sem->type,
                                               initial_val, 0);
      if (result != VK_SUCCESS) {
         vn_renderer_sync_destroy(perm_sync, alloc);
         return result;
   ...
   sem->permanent.type =
      perm_sync->sync_id ? VN_SYNC_TYPE_SYNC : VN_SYNC_TYPE_DEVICE_ONLY;
                  ...
                  uint64_t timeout)
{
   ...
   VkResult result =
      vn_renderer_wait(dev->instance->renderer, syncs, pWaitInfo->pValues,
                       pWaitInfo->semaphoreCount,
                       pWaitInfo->flags & VK_SEMAPHORE_WAIT_ANY_BIT, timeout);
                  ...
                  VkDeviceMemory *pMemory)
{
   ...
   result = vn_renderer_bo_create_memory(
      dev->instance->renderer, mem->size, mem->base.id,
      mem_type->propertyFlags, 0, alloc, VK_SYSTEM_ALLOCATION_SCOPE_OBJECT,
      &mem->bo);
   if (result != VK_SUCCESS) {
...
vn_EndCommandBuffer(VkCommandBuffer commandBuffer)
{
   ...
   vn_async_flush(instance);
   vn_renderer_submit(instance->renderer, &cmd->cs, NULL, 0, true);
