...
VkResult
vn_wsi_init(struct vn_physical_device *physical_device)
{
   return wsi_device_init(
      &physical_device->wsi_device,
      vn_physical_device_to_handle(physical_device), vn_wsi_proc_addr,
      &physical_device->instance->allocator, -1, NULL, false);
}

void
vn_wsi_fini(struct vn_physical_device *physical_device)
{
   wsi_device_finish(&physical_device->wsi_device,
                     &physical_device->instance->allocator);
}
