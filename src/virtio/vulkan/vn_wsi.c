...
vn_wsi_init(struct vn_physical_device *physical_dev)
{
   ...
   physical_dev->vk.wsi_device = &physical_dev->wsi_device;
...
vn_wsi_fini(struct vn_physical_device *physical_dev)
{
   const VkAllocationCallbacks *alloc =
      &physical_dev->instance->base.base.alloc;
   physical_dev->vk.wsi_device = NULL;
                      ...
                      VkSwapchainKHR *pSwapchain)
{
   ...
   VkResult result = wsi_CreateSwapchain(device, pCreateInfo,
                       ...
                       const VkAllocationCallbacks *pAllocator)
{
   ...
   wsi_DestroySwapchain(device, swapchain, pAllocator);
   if (VN_DEBUG(WSI))
