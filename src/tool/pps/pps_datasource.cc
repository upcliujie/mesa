...
// global counter id's:
// bits[0..7]  : device id
// bits[8..31] : counter id
#define DEV_ID_MASK 0xff
#define DEV_ID(n) (n & DEV_ID_MASK)
#define DEV_COUNTER_SHIFT 8
#define DEV_COUNTER(n) (n >> DEV_COUNTER_SHIFT)
#define COUNTER_ID(dev, counter) ((dev & DEV_ID_MASK) | (counter << DEV_COUNTER_SHIFT))
...
void GpuDataSource::OnSetup(const SetupArgs &args)
{
   ...
   for (auto &drm_device : drm_devices) {
      if (auto driver = Driver::create(std::move(drm_device))) {
   ...
   if (config.has_counter_ids()) {
      ...
      for (auto it = config.counter_ids(); it; ++it) {
         // for now device id is just index.
         // If we want to support changeable devices, we would need
         // to map from id to device, with persistent mapping
         auto &dri = drivers[DEV_ID(it->as_uint32())];
         dri->enable_counter(DEV_COUNTER(it->as_uint32()));
      }
   } else {
      // Enable all counters
      for (auto &dri : drivers) {
         dri->enable_all_counters();
      }
   }

   // Get sampling period
   // TODO: currently we limit sampling to the slowest rate. Add support
   // for sleeping until next device is ready, and provide counter updates
   // for each device as they are ready.
   auto min_sampling_period = std::chrono::nanoseconds(MIN_SAMPLING_PERIOD_NS);

   for (auto &dri : drivers) {
      auto dev_supported = std::chrono::nanoseconds(dri->get_min_sampling_period_ns());
...
{
   for (auto &dri : drivers) {
      dri->enable_perfcnt(time_to_sleep.count());
   }

   state = State::Start;
   PPS_LOG("OnStart finished");
}
...
void GpuDataSource::OnStop(const StopArgs &args)
{
   ...
   PPS_LOG("OnStop finished");
}

void GpuDataSource::register_data_source()
{
   static perfetto::DataSourceDescriptor dsd;
   dsd.set_name("gpu.metrics");
   Register(dsd);
}
   ...
   int32_t gpu_num)
{
   if (!group.counters.empty()) {
      ...
      block_desc->set_block_id(BLOCK_ID(gpu_num, group.id));

      // Associate counters to blocks
      for (auto id : group.counters) {
         block_desc->add_counter_ids(COUNTER_ID(gpu_num, id));
      }
   }
   ...
   Driver &driver)
{
   ...
   for (auto const &counter : counters) {
      ...
      spec->set_counter_id(COUNTER_ID(driver.drm_device.gpu_num, counter.id));
...
void GpuDataSource::trace_callback(TraceContext ctx)
{
   ...
   if (auto data_source = ctx.GetDataSourceLocked()) {
      ...
      if (data_source->time_to_sleep > data_source->time_to_trace) {
         ...
         PPS_LOG_ERROR("Trace took too long: %.2f ms", ms(data_source->time_to_trace));
         auto adjusted_time_to_sleep = data_source->time_to_sleep * 2;
...
//         data_source->time_to_sleep = adjusted_time_to_sleep;
   ...
   if (auto data_source = ctx.GetDataSourceLocked()) {
      ...
      PPS_LOG("Data source has been destroyed");
   }
}
