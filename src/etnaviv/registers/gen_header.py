...
class Enum(object):
	...
	def dump(self):
		...
		for (name, value) in self.values:
			if use_hex:
				print("\t%s = 0x%08x," % (self.name + "_" + name, value))
			else:
				print("\t%s = %d," % (self.name + "_" + name, value))
		print("};\n")
...
class Field(object):
	def __init__(self, name, low, high, shr, type, parser):
		...
		elif not self.type in builtin_types and not self.type in parser.enums and not self.type in parser.bitsets:
	...
	def ctype(self):
		if self.type == None:
			...
			type = "float"
			bits = self.high - self.low + 1
			maxi = ((1 << (bits - 1)) - 1)
			mini = -(1 << (bits - 1))
			maxf = maxi / (1 << self.radix)
			minf = mini / (1 << self.radix)

			val = "((int32_t)(val > %f ? %d : (val < %f ? %d : (val * %d.0f + 0.5f))))" % \
				(maxf, maxi, minf, mini, (1 << self.radix))
		elif self.type == "ufixed":
			type = "float"
			val = "((uint32_t)(val * %d.0f + 0.5f))" % (1 << self.radix)
...
class Bitset(object):
	...
	def dump(self, prefix=None):
		if prefix == None:
			prefix = self.name
		assert(not (len(self.fields) > 0 and len(self.values) > 0))
		for f in self.fields:
			...
			if not f.name and f.low == 0 and f.shr == 0 and not f.type in ["float", "fixed", "ufixed"] or not f.name:
				pass
			...
			for vname, value in f.values:
				tab_to("#define %s_%s" % (name, vname), "0x%08x" % (value << f.low))
		for vname, value in self.values:
			name = prefix + "_" + vname
			tab_to("#define %s" % name, "0x%08x" % value)
...
class Reg(object):
	def __init__(self, attrs, domain, array, stripe):
		self.name = attrs["name"]
		self.domain = domain
		self.array = array
		self.offset = int(attrs["offset"], 0)
		...
		for s in stripe:
			if s.length is None:
				continue
			self.stride.append(s.stride)

		if "length" in attrs:
			self.length_reg = int(attrs["length"], 0)
			self.stride.append(int(attrs["stride"], 0) if ("stride" in attrs) else 4)
	...
	def dump(self):
		if self.array:
			...
			if self.stride:
				print("static inline uint32_t %s(%s) { return 0x%08x + %s; }" % (name, \
				        ','.join("uint32_t i%d" % x for x in range(len(self.stride))), \
				        self.offset,
				        '+'.join("0x%x*i%d" % x[::-1] for x in enumerate(self.stride))))
			else:
				tab_to("#define %s" % name, "0x%08x" % self.offset)
			if self.length_reg:
				tab_to("#define %s__LEN" % name, "0x%08x" % self.length_reg)
...
class Stripe(object):
	...
	def dump(self):
		if self.length is None:
			return
		name = self.domain + "_" + self.name
		tab_to("#define %s__LEN" % name, "0x%08x" % self.length)
...
class Parser(object):
	...
	def prefix(self, want_domain=True):
		if len(self.current_stripe) > 0:
			return self.current_domain + "_" + '_'.join([x.name for x in self.current_stripe])
		elif self.current_prefix:
			return self.current_prefix + "_" + self.current_domain
		else:
			return self.current_domain if want_domain else None
	...
	def start_element(self, name, attrs):
		if name == "import":
			...
			self.current_stripe.append(Stripe(attrs, self.prefix()))
			self.file.append(self.current_stripe[-1])
		elif name == "enum":
			self.current_enum_value = 0
			self.current_enum = Enum(self.prefix(False),  attrs["name"])
			...
			if self.current_enum is not None:
				self.current_enum.values.append((attrs["name"], value))
			elif self.current_field is not None:
				self.current_field.values.append((attrs["name"], value))
			elif self.current_bitset is not None:
				self.current_bitset.values.append((attrs["name"], value))
			else:
				assert(False)
			...
			self.current_reg = Reg(attrs, self.prefix(), self.current_array, self.current_stripe)
