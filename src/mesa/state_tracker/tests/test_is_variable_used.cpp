...
static void
MakeNirVar(nir_builder& builder, int loc, const glsl_type *type) {
   nir_variable *variable = nir_variable_create(
      builder.shader, nir_var_shader_in, type, "var_0");
   variable->data.location = loc;
}
...
TEST_F(TestIsVariableUsed, VertexShader) {
   ...
   MakeNirVar(builder, VERT_ATTRIB_GENERIC10, glsl_vec4_type());
   mark_used(component_bitset, VERT_ATTRIB_GENERIC10 - VERT_ATTRIB_GENERIC0);

   MakeNirVar(builder, VERT_ATTRIB_GENERIC6, glsl_vec4_type());
   mark_used(component_bitset, VERT_ATTRIB_GENERIC6 - VERT_ATTRIB_GENERIC0);

   MakeNirVar(builder, VERT_ATTRIB_GENERIC3, glsl_float_type());
   mark_used(component_bitset, VERT_ATTRIB_GENERIC3 - VERT_ATTRIB_GENERIC0, 0);
