                            ...
                            const VkClearRect *rects)
{
   ...
   trace_start_sysmem_clear_all(cs == &cmd->cs ? &cmd->trace : &cmd->trace_per_tile, cs);
   ...
   trace_end_sysmem_clear_all(cs == &cmd->cs ? &cmd->trace : &cmd->trace_per_tile,
                              ...
                              const VkClearValue *value)
{
   ...
   trace_start_gmem_clear(&cmd->trace_per_tile, cs);
   ...
   trace_end_gmem_clear(&cmd->trace_per_tile, cs, att->format, att->samples);
}
                        ...
                        bool separate_stencil)
{
   ...
   trace_start_sysmem_clear(cs == &cmd->cs ? &cmd->trace : &cmd->trace_per_tile, cs);
   ...
   trace_end_sysmem_clear(cs == &cmd->cs ? &cmd->trace : &cmd->trace_per_tile, cs,
                          format, ops == &r3d_ops,
                          cmd->state.pass->attachments[a].samples);
}
                        ...
                        bool force_load)
{
   ...
   trace_start_gmem_load(&cmd->trace_per_tile, cs);
   ...
   trace_end_gmem_load(&cmd->trace_per_tile, cs, attachment->format, force_load);
}
                         ...
                         uint32_t gmem_a)
{
   ...
   trace_start_gmem_store(&cmd->trace_per_tile, cs);
   ...
   if (!unaligned && (a == gmem_a || blit_can_resolve(dst->format))) {
      ...
      trace_end_gmem_store(&cmd->trace_per_tile, cs, dst->format, true, false);
      return;
   }
   ...
   trace_end_gmem_store(&cmd->trace_per_tile, cs, dst->format, false, unaligned);
}
