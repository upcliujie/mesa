...
static void
resolve_system(struct tu_cmd_buffer *cmd,
               struct tu_cs *cs,
               VkFormat format,
               struct tu_image_view *src,
               struct tu_image_view *dst,
                  ...
                  const VkRect2D *rect)
{
   ...
   if (dst->image->vk_format == VK_FORMAT_D32_SFLOAT_S8_UINT) {
      resolve_system(cmd, cs, VK_FORMAT_D32_SFLOAT,
                     src, dst, layer_mask, layers, rect, false);
      resolve_system(cmd, cs, VK_FORMAT_S8_UINT,
                     src, dst, layer_mask, layers, rect, true);
   } else {
      resolve_system(cmd, cs, dst->image->vk_format,
                     src, dst, layer_mask, layers, rect, false);
   }
}
              ...
              uint32_t cpp)
{
   ...
   tu_cs_emit_regs(cs,
                   A6XX_SP_PS_2D_SRC_INFO(
                      ...
                      .samples_average = iview->image->samples > 1 &&
