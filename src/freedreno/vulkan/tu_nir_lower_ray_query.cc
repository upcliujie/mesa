...
static nir_def *
get_uav_index(struct rq_var *var, nir_instr *cur_instr)
{
   if (var->initialization &&
       nir_block_dominates(var->initialization->instr.block,
                           cur_instr->block)) {
      return var->uav_index;
   } else {
      return NULL;
   }
}
                    ...
                    nir_intrinsic_instr *intr)
{
   ...
   nir_def *tlas_base;
   if (var->uav_index) {
      tlas_base = load_tlas(b, NULL, var->uav_index, nir_imm_int(b, 0),
   ...
   nir_def *root_bvh_base = load_tlas(b, tlas_base, var->uav_index, nir_imm_int(b, 0),
                                      offsetof(struct tu_accel_struct_header,
                                               bvh_ptr), 2);
...
static void
insert_terminate_on_first_hit(nir_builder *b, nir_deref_instr *rq, bool break_on_terminate)
{
...
lower_rq_confirm_intersection(nir_builder *b, struct hash_table *ht, nir_intrinsic_instr *intr)
{
   ...
   insert_terminate_on_first_hit(b, rq, false);
}
...
{
   nir_metadata_require(nir_cf_node_get_function(&intr->instr.block->cf_node), nir_metadata_dominance);

   struct rq_var *var;
   nir_deref_instr *rq = get_rq_deref(b, ht, intr->src[0].ssa, &var);
   nir_def *uav_index = get_uav_index(var, &intr->instr);
                                   ...
                                   TU_INTERSECTION_TYPE_LEAF));
      {
         ...
         nir_push_else(b, NULL);
         {
            ...
            nir_push_else(b, NULL);
            {
               ...
               {
                  nir_copy_deref(b, closest, candidate);
                  insert_terminate_on_first_hit(b, rq, true);
               }
               nir_pop_if(b, NULL);
            }
            nir_pop_if(b, NULL);
...
lower_rq_proceed(nir_builder *b, struct hash_table *ht, nir_intrinsic_instr *intr)
{
   ...
   nir_push_if(b, nir_load_deref(b, rq_deref(b, rq, incomplete)));
   {
      ...
      nir_store_deref(b, rq_deref(b, rq, incomplete),
                      nir_iand(b, nir_load_deref(b, rq_deref(b, rq, incomplete)),
                               incomplete), 0x1);
   }
   nir_pop_if(b, NULL);
