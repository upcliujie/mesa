   ...
   VkFormatProperties3 *out_properties)
{
    ...
    * special codepath and that doesn't seem worth it. On a7xx we haven't
    * reverse-engineered the new fast-clear format to be able to invalidate
    * LRZ when writing to depth, so don't support depth at all.
    */
   if (!is_npot && vk_format != VK_FORMAT_D24_UNORM_S8_UINT &&
       (physical_device->info->chip == 6 ||
        tu6_pipe2depth(vk_format) == DEPTH6_NONE)) {
