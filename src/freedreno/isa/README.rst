...
In the first level of instruction encoding hiearchy, the high three bits
...
cat4 instructions), ie. GPR vs CONST vs relative GPR/CONST.
...
by name in multiple other places.  An expression is a simple
`RPN <https://en.wikipedia.org/wiki/Reverse_Polish_notation>` "program",
which can reference the values of other fields, and operate on them with
a limited/concise instruction set.  For example:

.. code-block: xml

   <expr name="#cat2-cat3-nop-encoding">
   	<doc>((SRC1_R != 0) || (SRC2_R != 0)) &amp;&amp; (REPEAT == 0)</doc>
   	<literal val="0"/>
   	<var name="SRC1_R"/>
   	<ne/>
   	<literal val="0"/>
...
The return value is the topmost value on the stack when the expression
returns.  In the case of ``<override>`` elements, the override applies
if the expression evaluates to non-zero.  In the case of ``<derived>``
fields, the expression evaulates to the value of the drived field.

TODO
----

 * More details about rules of resolving field names wrt. bitsets
   and nested bitsets, etc
 * Describe the possible ``<expr>`` opcodes.. skipped this so far
   because I'm still adding new ones as-needed.
