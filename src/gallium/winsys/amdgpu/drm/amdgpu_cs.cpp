                                     ...
                                     uint64_t *seq_no)
{
   ...
   /* Syncobj dependencies. */
   unsigned num_syncobj_dependencies = cs->syncobj_dependencies.num;
   /* allocate 1 extra for vm timeline syncobj. */
   uint32_t *syncobj_dependencies_list =
      (uint32_t*)alloca((num_syncobj_dependencies + 1) * sizeof(uint32_t));
   uint64_t *syncobj_dependencies_points_list =
      (uint64_t*)alloca((num_syncobj_dependencies + 1) * sizeof(uint64_t));
   if (num_syncobj_dependencies) {
      for (i = 0; i < num_syncobj_dependencies; i++) {
         ...
         syncobj_dependencies_points_list[i] = 0;
      }
   }
    ...
    */
    if (vm_timeline_seq_num && vm_timeline_seq_num != vm_timeline_seq_num_prev) {
       syncobj_dependencies_list[num_syncobj_dependencies] = aws->vm_timeline_syncobj;
      syncobj_dependencies_points_list[num_syncobj_dependencies] = vm_timeline_seq_num;
   ...
   r = amdgpu_userq_wait(aws->dev, userq->q_id,
                         (uint64_t)syncobj_dependencies_list, num_syncobj_dependencies,
                         (uint64_t)syncobj_dependencies_points_list, num_syncobj_dependencies,
                         (uint64_t)shared_buf_kms_handles, num_shared_buffers,
                         NULL, &num_fences, AMDGPU_USERQ_BO_WRITE);
   if (r)
