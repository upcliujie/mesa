...
static int amdgpu_bo_va_op_common(struct amdgpu_winsys *aws, amdgpu_bo_handle bo_handle,
                                  bool use_va_op_raw, uint64_t offset, uint64_t size,
                                  uint64_t addr, uint64_t flags, uint32_t ops,
                                  uint32_t sync_handle, uint64_t point)
{
   int r;
...
void amdgpu_bo_destroy(struct amdgpu_winsys *aws, struct pb_buffer_lean *_buf)
{
   ...
   if (bo->b.base.placement & RADEON_DOMAIN_VRAM_GTT) {
      amdgpu_bo_va_op_common(aws, bo->bo_handle, false, 0, bo->b.base.size,
                                                 ...
                                                 int heap)
{
   ...
   if (initial_domain & RADEON_DOMAIN_VRAM_GTT) {
      ...
      r = amdgpu_bo_va_op_common(aws, buf_handle, true, 0,  size, va, vm_flags,
...
static void amdgpu_bo_sparse_destroy(struct radeon_winsys *rws, struct pb_buffer_lean *_buf)
{
   ...
   r = amdgpu_bo_va_op_common(aws, NULL, true, 0,
                        ...
                        enum radeon_bo_flag flags)
{
   ...
   r = amdgpu_bo_va_op_common(aws, NULL, true, 0, map_size, gpu_address, AMDGPU_VM_PAGE_PRT,
                              AMDGPU_VA_OP_MAP, aws->vm_timeline_syncobj,
                              p_atomic_inc_return(&aws->vm_timeline_seq_num));
   if (r)
      goto error_va_map;
   ...
   if (commit) {
      while (va_page < end_va_page) {
         ...
         while (span_va_page < va_page) {
            ...
            r = amdgpu_bo_va_op_common(aws, backing->bo->bo_handle, true,
                                               ...
                                               bool is_prime_linear_buffer)
{
   ...
   r = amdgpu_bo_va_op_common(aws, result.buf_handle, true, 0, result.alloc_size, va,
                              AMDGPU_VM_PAGE_READABLE | AMDGPU_VM_PAGE_WRITEABLE |
                              AMDGPU_VM_PAGE_EXECUTABLE |
					    ...
					    enum radeon_bo_flag flags)
{
    ...
    if (amdgpu_bo_va_op_common(aws, buf_handle, false, 0, aligned_size, va,
                               AMDGPU_VM_PAGE_READABLE | AMDGPU_VM_PAGE_WRITEABLE |
