...
static int amdgpu_bo_va_op_common(struct amdgpu_winsys *aws, struct amdgpu_winsys_bo *bo,
                                  amdgpu_bo_handle bo_handle, bool use_va_op_raw,
                                  bool do_input_fence, uint64_t offset, uint64_t size,
                                  uint64_t addr, uint64_t flags, uint32_t ops, uint32_t sync_handle,
                                  uint64_t point)
{
   int r;
   ...
   if (aws->info.is_userq_enabled) {
      ...
      if (use_va_op_raw) {
         ...
         /* size is aligned and default flags set w.r.t raw function in libdrm. */
         r = amdgpu_bo_va_op2(bo_handle, offset, size, addr, flags, ops,
                               sync_handle, point, (uint64_t)&syncobj_arr, num_fences);
      }
      ...
      if (use_va_op_raw)
         ...
         r = amdgpu_bo_va_op(bo_handle, offset, size, addr, flags, ops);
   }
...
void amdgpu_bo_destroy(struct amdgpu_winsys *aws, struct pb_buffer_lean *_buf)
{
   ...
   if (bo->b.base.placement & RADEON_DOMAIN_VRAM_GTT) {
      amdgpu_bo_va_op_common(aws, amdgpu_winsys_bo(_buf), bo->bo_handle, false, true, 0,
                             bo->b.base.size, amdgpu_va_get_start_addr(bo->va_handle), 0,
                                                 ...
                                                 int heap)
{
   ...
   if (initial_domain & RADEON_DOMAIN_VRAM_GTT) {
      ...
      r = amdgpu_bo_va_op_common(aws, NULL, buf_handle, true, false, 0,  size, va, vm_flags,
...
static void amdgpu_bo_sparse_destroy(struct radeon_winsys *rws, struct pb_buffer_lean *_buf)
{
   ...
   r = amdgpu_bo_va_op_common(aws, amdgpu_winsys_bo(_buf), NULL, true, true, 0,
                        ...
                        enum radeon_bo_flag flags)
{
   ...
   r = amdgpu_bo_va_op_common(aws, NULL, NULL, true, false, 0, map_size, gpu_address,
   ...
   if (commit) {
      while (va_page < end_va_page) {
         ...
         while (span_va_page < va_page) {
            ...
            r = amdgpu_bo_va_op_common(aws, amdgpu_winsys_bo(buf), backing->bo->bo_handle, true, true,
      ...
      r = amdgpu_bo_va_op_common(aws, amdgpu_winsys_bo(buf), NULL, true, true, 0,
                                               ...
                                               bool is_prime_linear_buffer)
{
   ...
   r = amdgpu_bo_va_op_common(aws, NULL, result.buf_handle, true, false, 0, result.alloc_size, va,
                              AMDGPU_VM_PAGE_READABLE | AMDGPU_VM_PAGE_WRITEABLE |
                              AMDGPU_VM_PAGE_EXECUTABLE |
					    ...
					    enum radeon_bo_flag flags)
{
    ...
    if (amdgpu_bo_va_op_common(aws, NULL, buf_handle, false, false, 0, aligned_size, va,
                               AMDGPU_VM_PAGE_READABLE | AMDGPU_VM_PAGE_WRITEABLE |
