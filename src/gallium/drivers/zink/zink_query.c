...
struct zink_query {
   ...
   bool range_needs_reset; /* query is considered active by vk and cannot be destroyed */
...
static inline int get_num_starts(struct zink_query *q)
{
...
static struct zink_query_pool *find_or_allocate_qp(struct zink_context *ctx,
                                                   VkQueryType vk_query_type,
                                                   VkQueryPipelineStatisticFlags pipeline_stats)
{
   struct zink_screen *screen = zink_screen(ctx->base.screen);
                      ...
                      union pipe_query_result *result)
{
   ...
   struct pipe_transfer *xfer[PIPE_MAX_VERTEX_STREAMS] = {};
