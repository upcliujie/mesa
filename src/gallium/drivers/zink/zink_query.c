                      ...
                      union pipe_query_result *result)
{
   ...
   LIST_FOR_EACH_ENTRY(qbo, &query->buffers, list) {
      ...
      if (query->type == PIPE_QUERY_PRIMITIVES_GENERATED) {
         xfb_results = pipe_buffer_map_range(pctx, qbo->buffers[0], 0,
...
update_qbo(struct zink_context *ctx, struct zink_query *q)
{
   ...
   if (q->type == PIPE_QUERY_PRIMITIVES_EMITTED ||
      ...
      num_buffers = num_queries = PIPE_MAX_VERTEX_STREAMS;

   for (unsigned i = 0; i < num_queries; i++) {
      unsigned bufidx = num_queries == num_buffers ? i : 0;
      unsigned offset = is_timestamp ? 0 : get_buffer_offset(q, qbo->buffers[bufidx], query_id);
      VkQueryPool query_pool = q->query_pool[i];
      copy_pool_results_to_buffer(ctx, q, query_pool, query_id, zink_resource(qbo->buffers[bufidx]),
                                  offset,
