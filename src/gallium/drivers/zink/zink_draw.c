...
update_ubo_descriptors(struct zink_context *ctx, struct zink_descriptor_set *zds, struct zink_transition *transitions, int *num_transitions,
                       struct set *transition_hash, struct set *persistent, bool is_compute, bool cache_hit, uint32_t *dynamic_offsets, unsigned *dynamic_offset_idx)
{
   struct zink_program *pg = is_compute ? (struct zink_program *)ctx->curr_compute : (struct zink_program *)ctx->curr_program;
   struct zink_screen *screen = zink_screen(ctx->base.screen);
   ...
   *dynamic_offset_idx = dynamic_offset_count;
   ...
   return write_descriptors(ctx, zds, num_wds, wds, num_resources, resources, persistent, is_compute, cache_hit);
}
              ...
              unsigned num_draws)
{
    ...
    * this should also eventually trigger a stall if the app is going nuts with system memory
    */
   if (zink_curr_batch(ctx)->resource_size >= screen->total_mem / 10 / 4)
   ...
   if (dinfo->index_size > 0) {
      ...
      if (dindirect && dindirect->buffer) {
         ...
         if (dindirect->indirect_draw_count) {
             ...
             vkCmdDrawIndexedIndirectCount(batch->cmdbuf, indirect->buffer, dindirect->offset,
      ...
      if (so_target && screen->info.tf_props.transformFeedbackDraw) {
         ...
         if (dindirect->indirect_draw_count) {
             struct zink_resource *indirect_draw_count = zink_resource(dindirect->indirect_draw_count);
             zink_batch_reference_resource_rw(batch, indirect_draw_count, false);
             vkCmdDrawIndirectCount(batch->cmdbuf, indirect->buffer, dindirect->offset,
                                           indirect_draw_count->buffer, dindirect->indirect_draw_count_offset,
                                           dindirect->draw_count, dindirect->stride);
         } else
...
zink_launch_grid(struct pipe_context *pctx, const struct pipe_grid_info *info)
{
    ...
    * this should also eventually trigger a stall if the app is going nuts with system memory
    */
   if (batch->resource_size >= screen->total_mem / 10)
      zink_flush_compute(ctx);
