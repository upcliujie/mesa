             ...
             bool needs_drawid)
{
   ...
   if (dinfo->increment_draw_id && needs_drawid) {
      ...
      if (zink_screen(ctx->base.screen)->info.have_EXT_multi_draw)
         zink_screen(ctx->base.screen)->vk.CmdDrawMultiIndexedEXT(cmdbuf, num_draws, (void*)draws,
     ...
     bool needs_drawid)
{
   ...
   if (dinfo->increment_draw_id && needs_drawid) {
      ...
      if (needs_drawid)
         update_drawid(ctx, draw_id);
      if (zink_screen(ctx->base.screen)->info.have_EXT_multi_draw)
         zink_screen(ctx->base.screen)->vk.CmdDrawMultiEXT(cmdbuf, num_draws, (void*)draws,
