...
zink_emit_stream_output_targets(struct pipe_context *pctx)
{
   ...
   ctx->screen->vk.CmdBindTransformFeedbackBuffersEXT(batch->state->cmdbuf, 0, ctx->num_so_targets,
...
zink_bind_vertex_buffers(struct zink_batch *batch, struct zink_context *ctx)
{
   ...
   if (HAS_DYNAMIC_STATE)
      ctx->screen->vk.CmdBindVertexBuffers2EXT(batch->state->cmdbuf, 0,
             ...
             bool needs_drawid)
{
   ...
   if (dinfo->increment_draw_id && needs_drawid) {
      ...
      if (needs_drawid) {
         update_drawid(ctx, draw_id);
      if (HAS_MULTIDRAW)
         ctx->screen->vk.CmdDrawMultiIndexedEXT(cmdbuf, num_draws, (const VkMultiDrawIndexedInfoEXT*)draws,
     ...
     bool needs_drawid)
{
   ...
   if (dinfo->increment_draw_id && needs_drawid) {
      ...
      if (needs_drawid)
         update_drawid(ctx, draw_id);
      if (HAS_MULTIDRAW)
         ctx->screen->vk.CmdDrawMultiEXT(cmdbuf, num_draws, (const VkMultiDrawInfoEXT*)draws,
              ...
              unsigned num_draws)
{
   ...
   if (BATCH_CHANGED || ctx->vp_state_changed || (!HAS_DYNAMIC_STATE && pipeline_changed)) {
      ...
      if (HAS_DYNAMIC_STATE)
         ctx->screen->vk.CmdSetViewportWithCountEXT(batch->state->cmdbuf, ctx->vp_state.num_viewports, viewports);
      else
   ...
   if (BATCH_CHANGED || ctx->scissor_changed || ctx->vp_state_changed || (!HAS_DYNAMIC_STATE && pipeline_changed)) {
      ...
      if (HAS_DYNAMIC_STATE)
         ctx->screen->vk.CmdSetScissorWithCountEXT(batch->state->cmdbuf, ctx->vp_state.num_viewports, scissors);
      else
   ...
   if (HAS_DYNAMIC_STATE && (BATCH_CHANGED || ctx->dsa_state_changed)) {
      ctx->screen->vk.CmdSetDepthBoundsTestEnableEXT(batch->state->cmdbuf, dsa_state->hw_state.depth_bounds_test);
      ...
      ctx->screen->vk.CmdSetDepthTestEnableEXT(batch->state->cmdbuf, dsa_state->hw_state.depth_test);
      if (dsa_state->hw_state.depth_test)
         ctx->screen->vk.CmdSetDepthCompareOpEXT(batch->state->cmdbuf, dsa_state->hw_state.depth_compare_op);
      ctx->screen->vk.CmdSetDepthWriteEnableEXT(batch->state->cmdbuf, dsa_state->hw_state.depth_write);
      ctx->screen->vk.CmdSetStencilTestEnableEXT(batch->state->cmdbuf, dsa_state->hw_state.stencil_test);
      if (dsa_state->hw_state.stencil_test) {
         ctx->screen->vk.CmdSetStencilOpEXT(batch->state->cmdbuf, VK_STENCIL_FACE_FRONT_BIT,
         ...
         ctx->screen->vk.CmdSetStencilOpEXT(batch->state->cmdbuf, VK_STENCIL_FACE_BACK_BIT,
   ...
   if (HAS_DYNAMIC_STATE && (BATCH_CHANGED || rast_state_changed))
      ctx->screen->vk.CmdSetFrontFaceEXT(batch->state->cmdbuf, ctx->gfx_pipeline_state.front_face);
   ...
   if (HAS_DYNAMIC_STATE) {
      if (ctx->sample_locations_changed) {
         ...
         ctx->screen->vk.CmdSetSampleLocationsEXT(batch->state->cmdbuf, &loc);
      }
   ...
   if (index_size > 0) {
      if (dindirect && dindirect->buffer) {
         ...
         if (dindirect->indirect_draw_count) {
             ...
             ctx->screen->vk.CmdDrawIndexedIndirectCount(batch->state->cmdbuf, indirect->obj->buffer, dindirect->offset,
      ...
      if (so_target && ctx->screen->info.tf_props.transformFeedbackDraw) {
         ...
         ctx->screen->vk.CmdDrawIndirectByteCountEXT(batch->state->cmdbuf, dinfo->instance_count, dinfo->start_instance,
         ...
         if (dindirect->indirect_draw_count) {
             struct zink_resource *indirect_draw_count = zink_resource(dindirect->indirect_draw_count);
             zink_batch_reference_resource_rw(batch, indirect_draw_count, false);
             ctx->screen->vk.CmdDrawIndirectCount(batch->state->cmdbuf, indirect->obj->buffer, dindirect->offset,
                                           indirect_draw_count->obj->buffer, dindirect->indirect_draw_count_offset,
                                           dindirect->draw_count, dindirect->stride);
         } else
   ...
   if (STREAMOUT) {
      ...
      ctx->screen->vk.CmdEndTransformFeedbackEXT(batch->state->cmdbuf, 0, ctx->num_so_targets, counter_buffers, counter_buffer_offsets);
   }
   batch->has_work = true;
...
template <zink_multidraw HAS_MULTIDRAW, zink_dynamic_state HAS_DYNAMIC_STATE, bool BATCH_CHANGED,
          bool STREAMOUT, zink_drawid DRAWID, zink_basevertex BASEVERTEX>
static void
init_basevertex_functions(struct zink_context *ctx, pipe_draw_vbo_func draw_vbo_array[2][2][2][2][2][2])
{
   draw_vbo_array[HAS_MULTIDRAW][HAS_DYNAMIC_STATE][BATCH_CHANGED][STREAMOUT][DRAWID][BASEVERTEX] =
...
static void
init_drawid_functions(struct zink_context *ctx, pipe_draw_vbo_func draw_vbo_array[2][2][2][2][2][2])
{
   init_basevertex_functions<HAS_MULTIDRAW, HAS_DYNAMIC_STATE, BATCH_CHANGED, STREAMOUT, DRAWID, ZINK_NO_BASEVERTEX>(ctx, draw_vbo_array);
   init_basevertex_functions<HAS_MULTIDRAW, HAS_DYNAMIC_STATE, BATCH_CHANGED, STREAMOUT, DRAWID, ZINK_BASEVERTEX>(ctx, draw_vbo_array);
}

template <zink_multidraw HAS_MULTIDRAW, zink_dynamic_state HAS_DYNAMIC_STATE, bool BATCH_CHANGED,
          bool STREAMOUT>
static void
init_streamout_functions(struct zink_context *ctx, pipe_draw_vbo_func draw_vbo_array[2][2][2][2][2][2])
{
   init_drawid_functions<HAS_MULTIDRAW, HAS_DYNAMIC_STATE, BATCH_CHANGED, STREAMOUT, ZINK_NO_DRAWID>(ctx, draw_vbo_array);
   init_drawid_functions<HAS_MULTIDRAW, HAS_DYNAMIC_STATE, BATCH_CHANGED, STREAMOUT, ZINK_DRAWID>(ctx, draw_vbo_array);
}

template <zink_multidraw HAS_MULTIDRAW, zink_dynamic_state HAS_DYNAMIC_STATE, bool BATCH_CHANGED>
static void
init_batch_changed_functions(struct zink_context *ctx, pipe_draw_vbo_func draw_vbo_array[2][2][2][2][2][2])
{
   init_streamout_functions<HAS_MULTIDRAW, HAS_DYNAMIC_STATE, BATCH_CHANGED, false>(ctx, draw_vbo_array);
   init_streamout_functions<HAS_MULTIDRAW, HAS_DYNAMIC_STATE, BATCH_CHANGED, true>(ctx, draw_vbo_array);
}

template <zink_multidraw HAS_MULTIDRAW, zink_dynamic_state HAS_DYNAMIC_STATE>
static void
init_dynamic_state_functions(struct zink_context *ctx, pipe_draw_vbo_func draw_vbo_array[2][2][2][2][2][2])
{
...
template <zink_multidraw HAS_MULTIDRAW>
static void
init_multidraw_functions(struct zink_context *ctx, pipe_draw_vbo_func draw_vbo_array[2][2][2][2][2][2])
{
...
static void
init_all_draw_functions(struct zink_context *ctx, pipe_draw_vbo_func draw_vbo_array[2][2][2][2][2][2])
{
...
{
   pipe_draw_vbo_func draw_vbo_array[2][2][2][2][2][2]; //multidraw, dynamic state, batch changed, streamout, reads drawid,
                                                        //reads basevertex
