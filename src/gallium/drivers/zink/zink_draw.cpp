   ...
   set_foreach(need_barriers, he) {
      ...
      if (res->bind_count[is_compute]) {
         ...
         if (is_buffer) {
            ...
            /* calc ssbo mask below to avoid iterating twice */
         }
         if (is_compute) {
            pipeline = VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT;
            if (is_buffer && res->ssbo_bind_mask[PIPE_SHADER_COMPUTE])
               access |= VK_ACCESS_SHADER_READ_BIT;
         } else if (!pipeline) {
            if (is_buffer) {
               /* TODO: there are no write-only buffer descriptors without deeper shader analysis */
               VkPipelineStageFlags ssbo_flags = find_pipeline_bits(res->ssbo_bind_mask);
               if (ssbo_flags)
                  access |= VK_ACCESS_SHADER_READ_BIT;
               pipeline |= ssbo_flags;
