...
static void *get_real_lib()
{
    static void *real_lib = NULL;

    if (real_lib == NULL)
        real_lib = dlopen("/usr/lib/aarch64-linux-gnu/libc.so.6", RTLD_NOW | RTLD_GLOBAL);
...
dump_log(const char *format, ...)
{
   ...
   int dump_fd = open("rknpu.log", O_CREAT | O_RDWR | O_APPEND, S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP | S_IROTH | S_IWOTH);
...
map_bo(struct bo *bo)
{
   ...
   ret = ioctl(context.device_fd, DRM_IOCTL_RKNPU_MEM_MAP, &req);
   return mmap(NULL, bo->size, PROT_READ | PROT_WRITE, MAP_SHARED, context.device_fd, req.offset);
}
...
dump_buffer(const char *name, uint64_t dma_address, unsigned size)
{
   ...
   int fd = open(name, O_CREAT | O_RDWR | O_TRUNC, S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP | S_IROTH | S_IWOTH);
...
handle_submit(struct rknpu_submit *args, uint32_t *output_address)
{
   ...
   for(int i = args->task_start; i < args->task_start + args->task_number / 3; i++) {
      ...
      if (tasks[i].regcmd_addr == NULL)
         continue;

      char name[PATH_MAX];
      unsigned size = (tasks[i].regcfg_amount + RKNPU_PC_DATA_EXTRA_AMOUNT) * sizeof(uint64_t);
      ...
      *output_address = 0x0;
      ...
      unsigned offset = 0;
      struct bo *bo = find_bo(tasks[i].regcmd_addr + tasks[i].regcfg_offset, &offset);
      uint64_t *regcmd = map_bo(bo) + offset;
      for(int j = 0; j < tasks[i].regcfg_amount + RKNPU_PC_DATA_EXTRA_AMOUNT; j++) {
...
int ioctl(int fd, unsigned long request, ...)
{
   ...
   va_end(ap);
    ...
    real_ioctl = (real_ioctl_t)dlsym(get_real_lib(), "ioctl");
...
char *getenv(const char *name)
{
    ...
    real_getenv = (real_getenv_t)dlsym(get_real_lib(), "getenv");
