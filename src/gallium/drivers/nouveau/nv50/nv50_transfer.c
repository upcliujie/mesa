                        ...
                        uint32_t nblocksx, uint32_t nblocksy)
{
    ...
    * textures, which can really only happen with RGBA32
    * formats. Additionally, M2MF doesn't handle tiled textures in sysmem. Use
    * 2D blit engine for these.
    */
   bool eng2d = false;
   if (nouveau_bo_memtype(src->bo)) {
      if (src->domain == NOUVEAU_BO_GART)
         eng2d = true;
      if (src->width * cpp > 65536)
         eng2d = true;
   }
   if (nouveau_bo_memtype(dst->bo)) {
      if (dst->domain == NOUVEAU_BO_GART)
         eng2d = true;
