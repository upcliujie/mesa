...
void *
si_get_blitter_vs(struct si_context *sctx, enum blitter_attrib_type type, unsigned num_layers)
{
   ...
   if (num_layers > 1) {
      ...
      ureg_MOV(ureg, ureg_writemask(layer, TGSI_WRITEMASK_X), ureg_scalar(instance_id, TGSI_SWIZZLE_X));
   }
   ureg_END(ureg);
...
void *
si_create_dma_compute_shader(struct pipe_context *ctx, unsigned num_dwords_per_thread, bool dst_stream_cache_policy,
                             bool is_copy)
{
    ...
    */
   ureg_UMAD(ureg, store_addr, blk, ureg_imm1u(ureg, default_wave_size * num_mem_ops), tid);
   ...
   for (unsigned i = 0; i < num_mem_ops + load_store_distance; i++) {
      ...
      if (is_copy && i < num_mem_ops) {
         if (i) {
            ureg_UADD(ureg, load_addr, ureg_src(load_addr), ureg_imm1u(ureg, 4 * inst_dwords[i] * default_wave_size));
         }

         values[i] = ureg_src(ureg_DECL_temporary(ureg));
         struct ureg_dst dst = ureg_writemask(ureg_dst(values[i]), u_bit_consecutive(0, inst_dwords[i]));
         struct ureg_src srcs[] = {srcbuf, ureg_src(load_addr)};
         ureg_memory_insn(ureg, TGSI_OPCODE_LOAD, &dst, 1, srcs, 2, load_qualifier, TGSI_TEXTURE_BUFFER, 0);
      }

      if (d >= 0) {
         if (d) {
            ureg_UADD(ureg, store_addr, ureg_src(store_addr), ureg_imm1u(ureg, 4 * inst_dwords[d] * default_wave_size));
         }
         ...
         ureg_memory_insn(ureg, TGSI_OPCODE_STORE, &dst, 1, srcs, 2, store_qualifier, TGSI_TEXTURE_BUFFER, 0);
      }
   }
   ureg_END(ureg);
 ...
 */
void *
si_create_query_result_cs(struct si_context *sctx)
{
...
void *
si_clear_render_target_shader(struct pipe_context *ctx)
{
...
void *
si_clear_render_target_shader_1d_array(struct pipe_context *ctx)
{
   static const char text[] = "COMP\n"
...
void *
si_clear_12bytes_buffer_shader(struct pipe_context *ctx)
{
   static const char text[] = "COMP\n"
                              "PROPERTY CS_FIXED_BLOCK_WIDTH 64\n"
                              "PROPERTY CS_FIXED_BLOCK_HEIGHT 1\n"
   ...
   snprintf(final_text, sizeof(final_text), text, SI_COMPUTE_DST_CACHE_POLICY != L2_LRU ? ", STREAM_CACHE_POLICY" : "");
 ...
 */
void *
si_create_fmask_expand_cs(struct pipe_context *ctx, unsigned num_samples, bool is_array)
{
   ...
   ureg_UMAD(ureg, ureg_writemask(coord, TGSI_WRITEMASK_XY), ureg_swizzle(blk, 0, 1, 1, 1), ureg_imm2u(ureg, 8, 8),
   ...
   for (unsigned i = 0; i < num_samples; i++) {
      ...
      ureg_memory_insn(ureg, TGSI_OPCODE_LOAD, &sample[i], 1, srcs, 2, TGSI_MEMORY_RESTRICT, target, 0);
   }
   ...
   for (unsigned i = 0; i < num_samples; i++) {
      ...
      ureg_memory_insn(ureg, TGSI_OPCODE_STORE, &dst_image, 1, srcs, 2, TGSI_MEMORY_RESTRICT, target, 0);
   }
   ureg_END(ureg);
 ...
 */
void *
gfx11_create_sh_query_result_cs(struct si_context *sctx)
{
