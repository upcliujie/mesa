...
radeon_enc_slice_header_hevc(struct radeon_encoder *enc)
{
   ...
   if ((enc->enc_pic.hevc_deblock.loop_filter_across_slices_enabled) &&
       (!enc->enc_pic.hevc_deblock.deblocking_filter_disabled || enc->enc_pic.sample_adaptive_offset_enabled_flag)) {
      if (enc->enc_pic.sample_adaptive_offset_enabled_flag) {
         radeon_enc_flush_headers(enc);
         instruction[inst_index] = RENCODE_HEADER_INSTRUCTION_COPY;
         num_bits[inst_index] = enc->bits_output - bits_copied;
...
static void
radeon_enc_loop_filter_hevc(struct radeon_encoder *enc)
{
...
static void
radeon_enc_input_format(struct radeon_encoder *enc)
{
...
static void
radeon_enc_output_format(struct radeon_encoder *enc)
{
...
void
radeon_enc_2_0_init(struct radeon_encoder *enc)
{
