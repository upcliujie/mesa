                                         ...
                                         struct si_shader_args *args)
{
   ...
   nir_ssa_def *attr_address =
      sel->stage == MESA_SHADER_VERTEX && sel->info.base.vs.blit_sgprs_amd ? si_nir_load_internal_binding(b, args, SI_GS_ATTRIBUTE_RING, 4) : ac_nir_load_arg(b, &args->ac, args->gs_attr_address);
      ...
      for (unsigned stream = 0; stream < 4; stream++) {
         ...
         if (sel->screen->info.gfx_level >= GFX10) {
            ...
            unsigned data_format =
               sel->screen->info.gfx_level == GFX8 || sel->screen->info.gfx_level == GFX9 ? 0 : V_008F0C_BUF_DATA_FORMAT_32;
   ...
   switch (intrin->intrinsic) {
      ...
      case nir_intrinsic_load_lshs_vertex_stride_amd:
         if (stage == MESA_SHADER_VERTEX)
            ...
            replacement = sel->screen->info.gfx_level >= GFX9 && shader->is_monolithic ? nir_imm_int(b, key->ge.part.tcs.ls->info.lshs_vertex_stride) : nir_ishl_imm(b, GET_FIELD_NIR(VS_STATE_LS_OUT_VERTEX_SIZE), 2);
         else
      ...
      case nir_intrinsic_load_esgs_vertex_stride_amd:
         ...
         replacement = shader->is_monolithic ? nir_imm_int(b, key->ge.part.gs.es->info.esgs_vertex_stride / 4) : GET_FIELD_NIR(GS_STATE_ESGS_VERTEX_STRIDE);
         break;
      ...
      case nir_intrinsic_load_cull_small_prim_precision_amd: {
         nir_ssa_def *small_prim_precision =
            key->ge.opt.ngg_culling & SI_NGG_CULL_LINES ? GET_FIELD_NIR(GS_STATE_SMALL_PRIM_PRECISION_NO_AA) : GET_FIELD_NIR(GS_STATE_SMALL_PRIM_PRECISION);
      ...
      case nir_intrinsic_atomic_add_gs_invocation_count_amd: {
         ...
         enum pipe_statistics_query_index index =
            intrin->intrinsic == nir_intrinsic_atomic_add_gs_emit_prim_count_amd ? PIPE_STAT_QUERY_GS_PRIMITIVES : PIPE_STAT_QUERY_GS_INVOCATIONS;
      ...
      case nir_intrinsic_atomic_add_xfb_prim_count_amd: {
         ...
         unsigned offset = intrin->intrinsic == nir_intrinsic_atomic_add_gen_prim_count_amd ? offsetof(struct gfx11_sh_query_buffer_mem, stream[stream].generated_primitives) : offsetof(struct gfx11_sh_query_buffer_mem, stream[stream].emitted_primitives);
      ...
      case nir_intrinsic_load_color1: {
         ...
         nir_ssa_def *color[4];
         for (int i = 0; i < 4; i++) {
            color[i] = colors_read & BITFIELD_BIT(start + i) ? ac_nir_load_arg_at_offset(b, &args->ac, args->color_start, offset++) : nir_ssa_undef(b, 1, 32);
         }
