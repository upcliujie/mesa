...
static void
radeon_enc_sq_begin(struct radeon_encoder *enc)
{
...
static void
radeon_enc_sq_encode(struct radeon_encoder *enc)
{
   rvcn_sq_header(&enc->cs, &enc->sq, true);
   enc->mq_encode(enc);
   rvcn_sq_tail(&enc->cs, &enc->sq);
...
static void
radeon_enc_sq_destroy(struct radeon_encoder *enc)
{
   rvcn_sq_header(&enc->cs, &enc->sq, true);
   enc->mq_destroy(enc);
   rvcn_sq_tail(&enc->cs, &enc->sq);
...
radeon_enc_session_init(struct radeon_encoder *enc)
{
   ...
   case PIPE_VIDEO_FORMAT_AV1:
      ...
      enc->enc_pic.session_init.aligned_picture_width = align(enc->enc_pic.pic_width_in_luma_samples, 64);
      enc->enc_pic.session_init.aligned_picture_height = align(enc->enc_pic.pic_height_in_luma_samples, 16);
   ...
   if (av1_encoding) {
      enc->enc_pic.session_init.padding_width =
         enc->enc_pic.session_init.aligned_picture_width - enc->enc_pic.pic_width_in_luma_samples;
      enc->enc_pic.session_init.padding_height = enc->enc_pic.session_init.aligned_picture_height - av1_height;

      if (enc->enc_pic.enable_render_size)
         enc->enc_pic.enable_render_size =
            (enc->enc_pic.session_init.aligned_picture_width != enc->enc_pic.render_width) ||
            (enc->enc_pic.session_init.aligned_picture_height != enc->enc_pic.render_height);
   }
...
static uint32_t
radeon_enc_av1_calculate_temporal_id(uint32_t sequence_num, uint32_t max_layer)
{
...
static uint32_t
radeon_enc_av1_alloc_recon_slot(struct radeon_encoder *enc)
{
   uint32_t i;
...
static void
redeon_enc_av1_release_recon_slot(struct radeon_encoder *enc, uint32_t index, bool is_orphaned)
{
...
static uint32_t
radeon_enc_av1_alloc_curr_frame(struct radeon_encoder *enc, uint32_t frame_id, uint32_t temporal_id,
...
static void
radeon_enc_av1_release_ref_frame(struct radeon_encoder *enc, uint32_t index, bool is_recon_orphan)
{
   assert(index < (ARRAY_SIZE(enc->enc_pic.frames) - 1));

   redeon_enc_av1_release_recon_slot(enc, enc->enc_pic.frames[index].slot_id, is_recon_orphan);
   enc->enc_pic.frames[index].in_use = false;
}

/* save 1 recon slot in max temporal layer = 4 case */
static void
radeon_enc_av1_temporal_4_extra_release(struct radeon_encoder *enc, uint32_t temporal_id)
{
   uint32_t i = 0;
...
static void
radeon_enc_av1_pre_scan_frames(struct radeon_encoder *enc, uint32_t temporal_id)
{
   uint32_t i = 0;
...
static uint32_t
radeon_enc_av1_obtain_ref0_frame(struct radeon_encoder *enc, uint32_t temporal_id)
{
   uint32_t i = 0;
...
static void
radeon_enc_reset_av1_dpb_frames(struct radeon_encoder *enc)
{
...
static void
radeon_enc_av1_dpb_management(struct radeon_encoder *enc)
{
   ...
   if (pic->frame_type == PIPE_AV1_ENC_FRAME_TYPE_KEY) {
      ...
      pic->temporal_id = radeon_enc_av1_calculate_temporal_id(pic->frame_id, pic->num_temporal_layers - 1);
      pic->reference_frame_index = radeon_enc_av1_obtain_ref0_frame(enc, pic->temporal_id);
   ...
   pic->reference_delta_frame_id = pic->frame_id - pic->frames[pic->reference_frame_index].frame_id;
   current_slot = radeon_enc_av1_alloc_curr_frame(enc, pic->frame_id, pic->temporal_id, pic->frame_type);
   if (pic->frame_type == PIPE_AV1_ENC_FRAME_TYPE_KEY || pic->frame_type == PIPE_AV1_ENC_FRAME_TYPE_SWITCH ||
...
static void
radeon_enc_spec_misc_av1(struct radeon_encoder *enc)
{
   ...
   num_rows = PIPE_ALIGN_IN_BLOCK_SIZE(enc->enc_pic.session_init.aligned_picture_height, PIPE_AV1_ENC_SB_SIZE);
   num_columns = PIPE_ALIGN_IN_BLOCK_SIZE(enc->enc_pic.session_init.aligned_picture_width, PIPE_AV1_ENC_SB_SIZE);
...
static void
radeon_enc_cdf_default_table(struct radeon_encoder *enc)
{
...
uint8_t *
radeon_enc_av1_header_size_offset(struct radeon_encoder *enc)
{
...
static void
radeon_enc_av1_temporal_delimiter(struct radeon_encoder *enc)
{
   ...
   /* obu_extension_flag */
   use_extension_flag = (enc->enc_pic.num_temporal_layers) > 1 && (enc->enc_pic.temporal_id) > 0 ? 1 : 0;
...
static void
radeon_enc_av1_sequence_header(struct radeon_encoder *enc)
{
   uint8_t *size_offset;
   ...
   radeon_enc_code_fixed_bits(enc, enc->enc_pic.session_init.aligned_picture_width - 1, width_bits);
   /*  max_frame_height_minus_1  */
   radeon_enc_code_fixed_bits(enc, enc->enc_pic.session_init.aligned_picture_height - 1, height_bits);
...
static void
radeon_enc_av1_frame_header(struct radeon_encoder *enc, bool frame_header)
{
   uint32_t i;
   ...
   radeon_enc_code_fixed_bits(enc, 0, 1);
   /*  obu_type  */
   radeon_enc_code_fixed_bits(enc, frame_header ? RENCODE_OBU_TYPE_FRAME_HEADER : RENCODE_OBU_TYPE_FRAME, 4);
   /*  obu_extension_flag  */
   ...
   if (show_existing) {
      ...
      if (enc->enc_pic.frame_id_numbers_present)
         radeon_enc_code_fixed_bits(enc, enc->enc_pic.display_frame_id,
                                    RENCODE_AV1_DELTA_FRAME_ID_LENGTH + RENCODE_AV1_ADDITIONAL_FRAME_ID_LENGTH);
   } else {
      /*  frame_type  */
      ...
      if (!enc->enc_pic.disable_screen_content_tools) {
         ...
         allow_screen_content_tools = enc->enc_pic.av1_spec_misc.palette_mode_enable || enc->enc_pic.force_integer_mv;
      ...
      if ((!frame_is_intra || enc->enc_pic.refresh_frame_flags != 0xff) && error_resilient_mode &&
          enc->enc_pic.enable_order_hint)
      ...
      if (frame_is_intra) {
         ...
         for (i = 0; i < RENCDOE_AV1_REFS_PER_FRAME; i++) {
            ...
            if (enc->enc_pic.frame_id_numbers_present)
               radeon_enc_code_fixed_bits(enc, enc->enc_pic.reference_delta_frame_id - 1,
         ...
         if (frame_size_override && !error_resilient_mode)
            ...
            if (frame_size_override) {
               /*  frame_width_minus_1  */
               uint32_t used_bits = radeon_enc_value_bits(enc->enc_pic.session_init.aligned_picture_width - 1);
               radeon_enc_code_fixed_bits(enc, enc->enc_pic.session_init.aligned_picture_width - 1, used_bits);
               /*  frame_height_minus_1  */
               used_bits = radeon_enc_value_bits(enc->enc_pic.session_init.aligned_picture_height - 1);
               radeon_enc_code_fixed_bits(enc, enc->enc_pic.session_init.aligned_picture_height - 1, used_bits);
            }
            /*  render_and_frame_size_different  */
            radeon_enc_code_fixed_bits(enc, enc->enc_pic.enable_render_size ? 1 : 0, 1);
            if (enc->enc_pic.enable_render_size) {
...
static void
radeon_enc_av1_tile_group(struct radeon_encoder *enc)
{
   uint32_t extension_flag = enc->enc_pic.num_temporal_layers > 1 ? 1 : 0;
...
static void
radeon_enc_obu_instruction(struct radeon_encoder *enc)
{
   bool frame_header =
      !enc->enc_pic.stream_obu_frame || (enc->enc_pic.frame_type == PIPE_AV1_ENC_FRAME_TYPE_SHOW_EXISTING);
   ...
   radeon_enc_av1_bs_instruction_type(
      ...
      frame_header ? RENCODE_OBU_START_TYPE_FRAME_HEADER : RENCODE_OBU_START_TYPE_FRAME);
...
/* av1 encode params */
static void
radeon_enc_av1_encode_params(struct radeon_encoder *enc)
{
...
radeon_enc_ctx(struct radeon_encoder *enc)
{
   ...
   bool is_av1 = u_reduce_video_profile(enc->base.profile) == PIPE_VIDEO_FORMAT_AV1;
   ...
   for (int i = 0; i < RENCODE_MAX_NUM_RECONSTRUCTED_PICTURES; i++) {
      rvcn_enc_reconstructed_picture_t *pic = &enc->enc_pic.ctx_buf.reconstructed_pictures[i];
   ...
   for (int i = 0; i < RENCODE_MAX_NUM_RECONSTRUCTED_PICTURES; i++) {
      rvcn_enc_reconstructed_picture_t *pic = &enc->enc_pic.ctx_buf.pre_encode_reconstructed_pictures[i];
      RADEON_ENC_CS(pic->luma_offset);
      RADEON_ENC_CS(pic->chroma_offset);
      if (is_av1) {
         RADEON_ENC_CS(pic->av1.av1_cdf_frame_context_offset);
...
static void
radeon_enc_header_av1(struct radeon_encoder *enc)
{
...
void
radeon_enc_4_0_init(struct radeon_encoder *enc)
{
