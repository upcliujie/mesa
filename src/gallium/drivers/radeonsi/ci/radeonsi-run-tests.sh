...
if [[ $# != 4 ]]; then
    echo "Usage: $0 piglit_folder glcts_folder deqp_folder output_folder"
    echo "(use 'x' as glcts/deqp folder to skip those tests)"
    exit 1
fi
...
DEQP_PATH=$3
cd $PIGLIT_PATH
GPU_NAME=`bin/glinfo|grep GL_RENDER|sed "s/(TM)//"|cut -d\( -f2|cut -d, -f1| tr '[:upper:]' '[:lower:]'`
cd -
OUT=$4


if test "xGPU_NAME$" = "x"; then
    echo "Couldn't find GPU name using piglit/bin/glinfo."
    exit 1
fi
...
echo -e "Running tests for «$GPU_NAME»\n" | tee -a $logfile
