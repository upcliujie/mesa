...
if [[ $PIGLIT_PATH != "x" ]]; then
    ...
    grep -w Fail $OUT/piglit/results.csv > $OUT/new_baseline/${GPU_NAME}-piglit-quick-fail.csv
fi
...
if [[ $GLCTS_PATH != "x" ]]; then
    ...
    grep -w Fail $OUT/glcts/results.csv > $OUT/new_baseline/${GPU_NAME}-glcts-fail.csv
fi
...
if [[ $DEQP_PATH != "x" ]]; then
    ...
    do
        ...
        grep -w Fail $OUT/deqp-$subtest/results.csv > $OUT/new_baseline/${GPU_NAME}-deqp-$subtest-fail.csv
    done
fi
