...
static void si_emit_spi_config_cntl(struct si_context *sctx, struct radeon_cmdbuf *cs, bool enable);

static bool
si_sqtt_init_bo(struct si_context *sctx)
{
    ...
    * correctly. */
   sctx->sqtt->buffer_size = align64(sctx->sqtt->buffer_size, 1u << SQTT_BUFFER_ALIGN_SHIFT);
...
static void
si_emit_sqtt_start(struct si_context *sctx, struct radeon_cmdbuf *cs, uint32_t queue_family_index)
{
   ...
   for (unsigned se = 0; se < max_se; se++) {
      ...
      uint64_t data_va = ac_sqtt_get_data_va(&sctx->screen->info, sctx->sqtt, va, se);
      ...
      if (sctx->gfx_level >= GFX10) {
         uint32_t token_mask = V_008D18_REG_INCLUDE_SQDEC | V_008D18_REG_INCLUDE_SHDEC | V_008D18_REG_INCLUDE_GFXUDEC |
                               V_008D18_REG_INCLUDE_CONTEXT | V_008D18_REG_INCLUDE_COMP | V_008D18_REG_INCLUDE_CONFIG;
         ...
         if (sctx->gfx_level >= GFX11) {
            ...
            radeon_set_uconfig_reg(R_0367B4_SQ_THREAD_TRACE_MASK, S_0367B4_WTYPE_INCLUDE(shader_mask) |
                                                                     S_0367B4_SA_SEL(0) | S_0367B4_WGP_SEL(wgp) |
                                                                     S_0367B4_SIMD_SEL(0));

            radeon_set_uconfig_reg(
               R_0367B8_SQ_THREAD_TRACE_TOKEN_MASK,
               S_0367B8_REG_INCLUDE(token_mask) | S_0367B8_TOKEN_EXCLUDE(V_008D18_TOKEN_EXCLUDE_PERF));
         } else {
            ...
            radeon_set_privileged_config_reg(
               ...
               S_008D14_WTYPE_INCLUDE(shader_mask) | S_008D14_SA_SEL(0) | S_008D14_WGP_SEL(wgp) | S_008D14_SIMD_SEL(0));

            radeon_set_privileged_config_reg(
               R_008D18_SQ_THREAD_TRACE_TOKEN_MASK,
               S_008D18_REG_INCLUDE(token_mask) | S_008D18_TOKEN_EXCLUDE(V_008D18_TOKEN_EXCLUDE_PERF));
         }

         /* Should be emitted last (it enables thread traces). */
         uint32_t ctrl = S_008D1C_MODE(1) | S_008D1C_HIWATER(5) | S_008D1C_UTIL_TIMER(1) | S_008D1C_RT_FREQ(2) |
         ...
         case GFX10:
         case GFX10_3:
            ctrl |= S_008D1C_REG_STALL_EN(1) | S_008D1C_SPI_STALL_EN(1) | S_008D1C_SQ_STALL_EN(1) |
                    S_008D1C_REG_DROP_ON_STALL(0);
            radeon_set_privileged_config_reg(R_008D1C_SQ_THREAD_TRACE_CTRL, ctrl);
            break;
         case GFX11:
            ctrl |= S_0367B0_SPI_STALL_EN(1) | S_0367B0_SQ_STALL_EN(1) | S_0367B0_REG_AT_HWM(2);
         ...
         radeon_set_uconfig_reg(R_030CDC_SQ_THREAD_TRACE_BASE2, S_030CDC_ADDR_HI(shifted_va >> 32));

         radeon_set_uconfig_reg(R_030CC0_SQ_THREAD_TRACE_BASE, shifted_va);

         radeon_set_uconfig_reg(R_030CC4_SQ_THREAD_TRACE_SIZE, S_030CC4_SIZE(shifted_size));

         radeon_set_uconfig_reg(R_030CD4_SQ_THREAD_TRACE_CTRL, S_030CD4_RESET_BUFFER(1));
         ...
         radeon_set_uconfig_reg(R_030CEC_SQ_THREAD_TRACE_HIWATER, S_030CEC_HIWATER(4));

         if (sctx->gfx_level == GFX9) {
            /* Reset thread trace status errors. */
            radeon_set_uconfig_reg(R_030CE8_SQ_THREAD_TRACE_STATUS, S_030CE8_UTC_ERROR(0));
         }
   ...
   if (queue_family_index == AMD_IP_COMPUTE) {
      radeon_set_sh_reg(R_00B878_COMPUTE_THREAD_TRACE_ENABLE, S_00B878_THREAD_TRACE_ENABLE(1));
   } else {
...
static void
si_copy_sqtt_info_regs(struct si_context *sctx, struct radeon_cmdbuf *cs, unsigned se_index)
{
   ...
   if (sctx->gfx_level == GFX11) {
       ...
       */
      uint64_t data_va = ac_sqtt_get_data_va(&sctx->screen->info, sctx->sqtt, va, se_index);
...
static void
si_emit_sqtt_stop(struct si_context *sctx, struct radeon_cmdbuf *cs, uint32_t queue_family_index)
{
   unsigned max_se = sctx->screen->info.max_se;
   ...
   if (queue_family_index == AMD_IP_COMPUTE) {
      radeon_set_sh_reg(R_00B878_COMPUTE_THREAD_TRACE_ENABLE, S_00B878_THREAD_TRACE_ENABLE(0));
   } else {
      radeon_emit(PKT3(PKT3_EVENT_WRITE, 0, 0));
   ...
   if (sctx->screen->info.has_sqtt_rb_harvest_bug) {
       ...
       * doesn't work. */
      sctx->flags |= SI_CONTEXT_FLUSH_AND_INV_CB | SI_CONTEXT_FLUSH_AND_INV_DB | SI_CONTEXT_CS_PARTIAL_FLUSH;
   ...
   for (unsigned se = 0; se < max_se; se++) {
      ...
      if (sctx->gfx_level >= GFX10) {
         uint32_t tt_status_reg =
            sctx->gfx_level >= GFX11 ? R_0367D0_SQ_THREAD_TRACE_STATUS : R_008D20_SQ_THREAD_TRACE_STATUS;
         if (!sctx->screen->info.has_sqtt_rb_harvest_bug) {
...
static void
si_sqtt_start(struct si_context *sctx, int family, struct radeon_cmdbuf *cs)
{
   struct radeon_winsys *ws = sctx->ws;
   ...
   ws->cs_add_buffer(cs, sctx->sqtt->bo, RADEON_USAGE_READWRITE, RADEON_DOMAIN_VRAM);
   if (sctx->spm.bo)
      ws->cs_add_buffer(cs, sctx->spm.bo, RADEON_USAGE_READWRITE, RADEON_DOMAIN_VRAM);

   si_cp_dma_wait_for_idle(sctx, cs);

   /* Make sure to wait-for-idle before starting SQTT. */
   sctx->flags |= SI_CONTEXT_PS_PARTIAL_FLUSH | SI_CONTEXT_CS_PARTIAL_FLUSH | SI_CONTEXT_INV_ICACHE |
                  SI_CONTEXT_INV_SCACHE | SI_CONTEXT_INV_VCACHE | SI_CONTEXT_INV_L2 | SI_CONTEXT_PFP_SYNC_ME;
   sctx->emit_cache_flush(sctx, cs);
...
static void
si_sqtt_stop(struct si_context *sctx, int family, struct radeon_cmdbuf *cs)
{
   struct radeon_winsys *ws = sctx->ws;
...
static void
si_sqtt_init_cs(struct si_context *sctx)
{
   struct radeon_winsys *ws = sctx->ws;

   /* Thread trace start CS (only handles AMD_IP_GFX). */
   sctx->sqtt->start_cs[AMD_IP_GFX] = CALLOC_STRUCT(radeon_cmdbuf);
   if (!ws->cs_create(sctx->sqtt->start_cs[AMD_IP_GFX], sctx->ctx, AMD_IP_GFX, NULL, NULL, 0)) {
   ...
   if (!ws->cs_create(sctx->sqtt->stop_cs[AMD_IP_GFX], sctx->ctx, AMD_IP_GFX, NULL, NULL, 0)) {
      free(sctx->sqtt->start_cs[AMD_IP_GFX]);
      sctx->sqtt->start_cs[AMD_IP_GFX] = NULL;
...
static void
si_begin_sqtt(struct si_context *sctx, struct radeon_cmdbuf *rcs)
{
...
static void
si_end_sqtt(struct si_context *sctx, struct radeon_cmdbuf *rcs)
{
...
static bool
si_get_sqtt_trace(struct si_context *sctx, struct ac_sqtt_trace *sqtt)
{
   unsigned max_se = sctx->screen->info.max_se;

   memset(sqtt, 0, sizeof(*sqtt));

   sctx->sqtt->ptr = sctx->ws->buffer_map(sctx->ws, sctx->sqtt->bo, NULL, PIPE_MAP_READ);
   ...
   if (!ac_sqtt_get_trace(sctx->sqtt, &sctx->screen->info, sqtt)) {
      ...
      for (unsigned se = 0; se < max_se; se++) {
         ...
         if (!ac_is_sqtt_complete(&sctx->screen->info, sctx->sqtt, info)) {
            uint32_t expected_size = ac_get_expected_buffer_size(&sctx->screen->info, info);
...
bool
si_init_sqtt(struct si_context *sctx)
{
   ...
   sctx->sqtt->buffer_size = debug_get_num_option("AMD_THREAD_TRACE_BUFFER_SIZE", 32 * 1024) * 1024;
   ...
   if (sctx->gfx_level >= GFX10 && debug_get_bool_option("AMD_THREAD_TRACE_SPM", sctx->gfx_level < GFX11)) {
...
void
si_destroy_sqtt(struct si_context *sctx)
{
   struct si_screen *sscreen = sctx->screen;
   ...
   struct rgp_pso_correlation *pso_correlation = &sctx->sqtt->rgp_pso_correlation;
...
void
si_handle_sqtt(struct si_context *sctx, struct radeon_cmdbuf *rcs)
{
   /* Should we enable SQTT yet? */
   if (!sctx->sqtt_enabled) {
      bool frame_trigger = num_frames == sctx->sqtt->start_frame;
      bool file_trigger = false;
      if (sctx->sqtt->trigger_file && access(sctx->sqtt->trigger_file, W_OK) == 0) {
      ...
      if (frame_trigger || file_trigger) {
         ...
         sctx->ws->fence_wait(sctx->ws, sctx->last_gfx_fence, OS_TIMEOUT_INFINITE);
      ...
      if (sctx->ws->fence_wait(sctx->ws, sctx->last_sqtt_fence, OS_TIMEOUT_INFINITE) &&
          si_get_sqtt_trace(sctx, &sqtt_trace)) {
         struct ac_spm_trace spm_trace;

         /* Map the SPM counter buffer */
         if (sctx->spm.bo) {
            sctx->spm.ptr = sctx->ws->buffer_map(sctx->ws, sctx->spm.bo, NULL, PIPE_MAP_READ | RADEON_MAP_TEMPORARY);
            ac_spm_get_trace(&sctx->spm, &spm_trace);
         }

         ac_dump_rgp_capture(&sctx->screen->info, &sqtt_trace, sctx->spm.bo ? &spm_trace : NULL);
...
static void
si_emit_sqtt_userdata(struct si_context *sctx, struct radeon_cmdbuf *cs, const void *data, uint32_t num_dwords)
{
...
static void
si_emit_spi_config_cntl(struct si_context *sctx, struct radeon_cmdbuf *cs, bool enable)
{
   radeon_begin(cs);
...
void
si_sqtt_write_event_marker(struct si_context *sctx, struct radeon_cmdbuf *rcs, enum rgp_sqtt_marker_event_type api_type,
...
void
si_write_user_event(struct si_context *sctx, struct radeon_cmdbuf *rcs, enum rgp_sqtt_marker_user_event_type type,
                    const char *str, int len)
{
   if (type == UserEventPop) {
      ...
      si_emit_sqtt_userdata(sctx, rcs, buffer, sizeof(marker) / 4 + marker.length / 4);
   }
}

bool
si_sqtt_pipeline_is_registered(struct ac_sqtt *sqtt, uint64_t pipeline_hash)
{
   simple_mtx_lock(&sqtt->rgp_pso_correlation.lock);
   list_for_each_entry_safe (struct rgp_pso_correlation_record, record, &sqtt->rgp_pso_correlation.record, list) {
...
static bool
si_sqtt_add_code_object(struct si_context *sctx, struct si_sqtt_fake_pipeline *pipeline, bool is_compute)
{
   struct rgp_code_object *code_object = &sctx->sqtt->rgp_code_object;
...
si_sqtt_register_pipeline(struct si_context *sctx, struct si_sqtt_fake_pipeline *pipeline, bool is_compute)
{
   ...
   result = ac_sqtt_add_code_object_loader_event(sctx->sqtt, pipeline->code_hash, pipeline->bo->gpu_address);
   if (!result)
      return false;
...
void
si_sqtt_describe_pipeline_bind(struct si_context *sctx, uint64_t pipeline_hash, int bind_point)
{
