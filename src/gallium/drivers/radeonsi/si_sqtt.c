...
si_thread_trace_init_bo(struct si_context *sctx)
{
    ...
    * correctly. */
   sctx->screen->thread_trace->buffer_size = align64(sctx->screen->thread_trace->buffer_size,
   ...
   size += sctx->screen->thread_trace->buffer_size * 4;

   sctx->screen->thread_trace->bo =
      ws->buffer_create(ws, size, 4096,
                        RADEON_DOMAIN_VRAM,
                        RADEON_FLAG_NO_INTERPROCESS_SHARING |
                        RADEON_FLAG_GTT_WC |
                        RADEON_FLAG_NO_SUBALLOC);
   if (!sctx->screen->thread_trace->bo)
      return false;
                           ...
                           uint32_t queue_family_index)
{
   ...
   uint32_t shifted_size = sctx->screen->thread_trace->buffer_size >> SQTT_BUFFER_ALIGN_SHIFT;
   unsigned max_se = sscreen->info.max_se;

   for (unsigned se = 0; se < max_se; se++) {
      uint64_t va = sctx->ws->buffer_get_virtual_address(sctx->screen->thread_trace->bo);
      uint64_t data_va = ac_thread_trace_get_data_va(sctx->screen->thread_trace, va, se);
...
si_thread_trace_stop(struct si_context *sctx, int family, struct radeon_cmdbuf *cs)
{
   ...
   switch (family) {
   case RING_GFX:
      radeon_emit(sctx->screen->thread_trace->stop_cs[family], PKT3(PKT3_CONTEXT_CONTROL, 1, 0));
      radeon_emit(sctx->screen->thread_trace->stop_cs[family], CC0_UPDATE_LOAD_ENABLES(1));
      radeon_emit(sctx->screen->thread_trace->stop_cs[family], CC1_UPDATE_SHADOW_ENABLES(1));
      break;
   case RING_COMPUTE:
      radeon_emit(sctx->screen->thread_trace->stop_cs[family], PKT3(PKT3_NOP, 0, 0));
      radeon_emit(sctx->screen->thread_trace->stop_cs[family], 0);
      break;
   }
   ws->cs_add_buffer(cs,
                     sctx->screen->thread_trace->bo,
...
si_thread_trace_init_cs(struct si_context *sctx)
{
   ...
   /* Thread trace start CS (only RING_GFX). */
   sctx->screen->thread_trace->start_cs[RING_GFX] = calloc(sizeof(struct radeon_cmdbuf), 1);
   if (!ws->cs_create(sctx->screen->thread_trace->start_cs[RING_GFX],
                      sctx->ctx, RING_GFX, NULL, NULL, 0)) {
      free(sctx->screen->thread_trace->start_cs[RING_GFX]);
      sctx->screen->thread_trace->start_cs[RING_GFX] = NULL;
      return;
   }

   si_thread_trace_start(sctx, RING_GFX, sctx->screen->thread_trace->start_cs[RING_GFX]);

   /* Thread trace stop CS. */
   sctx->screen->thread_trace->stop_cs[RING_GFX] = calloc(sizeof(struct radeon_cmdbuf), 1);
   if (!ws->cs_create(sctx->screen->thread_trace->stop_cs[RING_GFX],
                      sctx->ctx, RING_GFX, NULL, NULL, 0)) {
      free(sctx->screen->thread_trace->start_cs[RING_GFX]);
      sctx->screen->thread_trace->start_cs[RING_GFX] = NULL;
      free(sctx->screen->thread_trace->stop_cs[RING_GFX]);
      sctx->screen->thread_trace->stop_cs[RING_GFX] = NULL;
      return;
   }

   si_thread_trace_stop(sctx, RING_GFX, sctx->screen->thread_trace->stop_cs[RING_GFX]);
}

static void
si_begin_thread_trace(struct si_context *sctx, struct radeon_cmdbuf *rcs)
{
   struct radeon_cmdbuf *cs = sctx->screen->thread_trace->start_cs[RING_GFX];
...
{
   struct radeon_cmdbuf *cs = sctx->screen->thread_trace->stop_cs[RING_GFX];
   sctx->ws->cs_flush(cs, 0, &sctx->screen->last_sqtt_fence);
}
                    ...
                    struct ac_thread_trace *thread_trace)
{
   ...
   sctx->ws->buffer_wait(sctx->screen->thread_trace->bo, PIPE_TIMEOUT_INFINITE,
                         RADEON_USAGE_READ);

   sctx->screen->thread_trace->ptr = sctx->ws->buffer_map(sctx->screen->thread_trace->bo, NULL, PIPE_MAP_READ_WRITE);


   if (!sctx->screen->thread_trace->ptr)
      return false;

   void *thread_trace_ptr = sctx->screen->thread_trace->ptr;

   for (unsigned se = 0; se < max_se; se++) {
      uint64_t info_offset = ac_thread_trace_get_info_offset(se);
      uint64_t data_offset = ac_thread_trace_get_data_offset(sctx->screen->thread_trace, se);
...
bool
si_init_thread_trace(struct si_screen *sscreen) {
   fprintf(stderr, "*************************************************\n");
   ...
   sscreen->thread_trace = calloc(1, sizeof(struct ac_thread_trace_data));
   ...
   sscreen->thread_trace->buffer_size = debug_get_num_option("AMD_THREAD_TRACE_BUFFER_SIZE", 1024) * 1024;
   sscreen->thread_trace->start_frame = 10;

   const char *trigger_file = getenv("AMD_THREAD_TRACE_TRIGGER");
   if (trigger_file) {
      sscreen->thread_trace->trigger_file = strdup(trigger_file);
      sscreen->thread_trace->start_frame = -1;
   }
...
void si_destroy_thread_trace(struct si_screen *sscreen) {
   struct pb_buffer *bo = sscreen->thread_trace->bo;
   pb_reference(&bo, NULL);

   if (sscreen->thread_trace->trigger_file)
      free(sscreen->thread_trace->trigger_file);
   sscreen->ws->cs_destroy(sscreen->thread_trace->start_cs[RING_GFX]);
   sscreen->ws->cs_destroy(sscreen->thread_trace->stop_cs[RING_GFX]);
   free(sscreen->thread_trace);
   sscreen->thread_trace = NULL;
}
...
si_handle_thread_trace(struct si_context *sctx, struct radeon_cmdbuf *rcs) {
   ...
   if (!sctx->screen->thread_trace_enabled) {
      bool frame_trigger = num_frames == sctx->screen->thread_trace->start_frame;
      bool file_trigger = false;
      if (
          sctx->screen->thread_trace->trigger_file &&
          access(sctx->screen->thread_trace->trigger_file, W_OK) == 0) {
         if (unlink(sctx->screen->thread_trace->trigger_file) == 0) {
            file_trigger = true;
         } else {
      ...
      if (frame_trigger || file_trigger) {
         ...
         sctx->screen->thread_trace_enabled = true;
         sctx->screen->thread_trace->start_frame = -1;
      }
   } else {
      ...
      sctx->screen->thread_trace_enabled = false;
      sctx->screen->thread_trace->start_frame = -1;
      assert (sctx->screen->last_sqtt_fence);

      /* Wait for SQTT to finish and read back the bo */
      if (sctx->ws->fence_wait(sctx->ws, sctx->screen->last_sqtt_fence, PIPE_TIMEOUT_INFINITE) &&
