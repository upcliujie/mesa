...
impl NirShader {
    pub fn new(nir: *mut nir_shader) -> Option<Self> {
        match NonNull::new(nir) {
            Some(nir) => Some(Self { nir: nir }),
            None => None,
        }
    }
    ...
    pub fn pass1<A>(
        &self,
        pass: unsafe extern "C" fn(*mut nir_shader, a: A) -> bool,
        a: A,
    ) -> bool {
    ...
    pub fn pass2<A, B>(
        &self,
        pass: unsafe extern "C" fn(*mut nir_shader, a: A, b: B) -> bool,
        a: A,
        b: B,
    ) -> bool {
    ...
    pub fn pass3<A, B, C>(
        &self,
        pass: unsafe extern "C" fn(*mut nir_shader, a: A, b: B, c: C) -> bool,
        a: A,
        b: B,
        c: C,
    ) -> bool {
    ...
    pub fn structurize(&self) {
        self.pass0(nir_lower_goto_ifs);
        self.pass0(nir_opt_dead_cf);
    }

    pub fn inline(&self, libclc: &NirShader) {
        self.pass1(
    ...
    pub fn remove_non_entrypoints(&self) {
        unsafe { nir_remove_non_entrypoints(self.nir.as_ptr()) };
    }

    pub fn variables(&self) -> ExecListIter<nir_variable> {
        ExecListIter::new(
            &unsafe { self.nir.as_ref() }.variables,
    ...
    ) -> impl Iterator<Item = &nir_variable> {
        self.variables()
            .filter(move |v| v.data.mode() & mode != nir_variable_mode(0))
    }
