...
pub trait CheckedMutCLType<CL> {
    fn check<'l>(&'l self) -> Result<&'l mut CL, cl_int>;
}
...
#[macro_export]
macro_rules! decl_cl_type {
    ($cl: ident, $i: ty, $r: ident, $err: ident) => {
        #[repr(C)]
        #[allow(non_camel_case_types)]
        pub struct $cl {
            pub base: self::rusticl_opencl_gen::$cl,
            // this is the API ref counter, we don't have to modify it on internal references and we
            // don't even need the cl object internally.
            // We are just screwed once the client hands in an object already fred.
            //
            // We _could_ prevent freeing as long as our internal object still has references
            // excluding the cl object, but...
            refs: ::std::sync::atomic::AtomicU32,
            inner: $r,
        }

        impl $cl {
            pub fn new(inner: $r) -> Box<Self> {
                Box::new(Self {
                    base: self::rusticl_opencl_gen::$cl {
                        dispatch: &crate::api::icd::DISPATCH,
                    },
            ...
            pub fn refs(&self) -> u32 {
                self.refs.load(::std::sync::atomic::Ordering::Relaxed)
            }
        }

        impl ::std::ops::Deref for $cl {
            type Target = $r;
        ...
        impl ::std::ops::DerefMut for $cl {
            fn deref_mut(&mut self) -> &mut Self::Target {
                &mut self.inner
            }
        }

        impl crate::api::util::CheckedCLType<$cl> for *const self::rusticl_opencl_gen::$cl {
            fn check<'l>(&'l self) -> Result<&'l $cl, cl_int> {
                let v = unsafe { ((*self) as *const $cl).as_ref().ok_or($err)? };
                if !::std::ptr::eq(v.base.dispatch, &crate::api::icd::DISPATCH) {
                    return Err($err);
                }
                Ok(v)
            }
        ...
        impl crate::api::util::CheckedMutCLType<$cl> for *mut self::rusticl_opencl_gen::$cl {
            fn check<'l>(&'l self) -> Result<&'l mut $cl, cl_int> {
                let v = unsafe { ((*self) as *mut $cl).as_mut().ok_or($err)? };
                if !::std::ptr::eq(v.base.dispatch, &crate::api::icd::DISPATCH) {
                    return Err($err);
                }
                Ok(v)
        ...
        // convenience trait impl so one doesn't have to be aware of the deref trickery
        impl<'a> ::std::iter::FromIterator<&'a $cl> for Vec<$r> {
            fn from_iter<I: IntoIterator<Item = &'a $cl>>(iter: I) -> Self {
                iter.into_iter()
        ...
        impl<'a> ::std::iter::FromIterator<&'a $cl> for Vec<&'a $r> {
            fn from_iter<I: IntoIterator<Item = &'a $cl>>(iter: I) -> Self {
                iter.into_iter()
                    .map(|i| ::std::ops::Deref::deref(i))
                    .collect()
            }
        }

        impl<'a> ::std::iter::FromIterator<&'a mut $cl> for Vec<$r> {
            fn from_iter<I: IntoIterator<Item = &'a mut $cl>>(iter: I) -> Self {
                iter.into_iter()
                    .map(|i| ::std::ops::Deref::deref(i).clone())
                    .collect()
            }
        ...
        impl<'a> ::std::iter::FromIterator<&'a mut $cl> for Vec<&'a $r> {
            fn from_iter<I: IntoIterator<Item = &'a mut $cl>>(iter: I) -> Self {
                iter.into_iter()
                    .map(|i| ::std::ops::Deref::deref(i))
                    .collect()
        ...
        impl ::std::cmp::PartialEq for $i {
            fn eq(&self, other: &Self) -> bool {
                self.cl == other.cl
            }
        }

        impl ::std::cmp::PartialEq for &$cl {
            fn eq(&self, other: &Self) -> bool {
                self.cl == other.cl
            }
        }

        impl ::std::cmp::PartialEq for &mut $cl {
            fn eq(&self, other: &Self) -> bool {
                self.cl == other.cl
            }
        }

        impl ::std::cmp::Eq for $i {}
        impl ::std::cmp::Eq for &$cl {}
        impl ::std::cmp::Eq for &mut $cl {}

        pub type $r = ::std::sync::Arc<$i>;
    };
}

#[macro_export]
macro_rules! init_cl_type {
    ($n: ident, $cl: ident) => {{
        // In order to not have to use unsafe blocks we have to become tricky

        // 1. create the CL object with the Arc
        let mut clbox = <$cl>::new(::std::sync::Arc::new($n));
        // 2. save pointer to CL object. Using get_mut is safe here as only one reference exists.
        ::std::sync::Arc::get_mut(&mut clbox).unwrap().cl =
            &mut *clbox as *mut $cl as *mut self::rusticl_opencl_gen::$cl;
        // 3. clone the reference
        let res = clbox.clone();
        // 4. leak the pointer
        Box::into_raw(clbox);
        // We could use into_raw realier, but then we'd need to derefence a pointer
        res
    }};
}
...
pub fn check_cl_objs<'a, CL: 'a + Deref, T: 'a + CheckedCLType<CL>>(
    objs: *const T,
    count: cl_uint,
) -> Result<Vec<&'a <CL as Deref>::Target>, cl_int>
where
    &'a <CL as Deref>::Target: PartialEq,
{
    ...
    for i in 0..count as usize {
        unsafe {
            res.push((*objs.add(i)).check()?.deref());
        }
    }
    res.dedup();
    Ok(res)
...
pub fn check_cl_objs_mut<'a, CL: 'a + DerefMut, T: 'a + CheckedMutCLType<CL>>(
    objs: *const T,
    count: cl_uint,
) -> Result<Vec<&'a mut <CL as Deref>::Target>, cl_int>
where
    &'a mut <CL as Deref>::Target: PartialEq,
{
    // CL spec requires validation for obj arrays, both values have to make sense
    if objs.is_null() && count > 0 || !objs.is_null() && count == 0 {
        return Err(CL_INVALID_VALUE)?;
    ...
    for i in 0..count as usize {
        unsafe {
            res.push((*objs.add(i)).check()?.deref_mut());
        }
    }
    res.dedup();
    Ok(res)
