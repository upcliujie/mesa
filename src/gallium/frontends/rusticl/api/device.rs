...
pub fn get_devs_for_type(device_type: cl_device_type) -> Vec<&'static Arc<Device>> {
    devs().iter()
      .filter(|d| match device_type as u32 {
         ...
         CL_DEVICE_TYPE_CPU => device_type & d.device_type(),
         CL_DEVICE_TYPE_CUSTOM => device_type & d.device_type(),
         CL_DEVICE_TYPE_DEFAULT => device_type & d.device_type(),
         CL_DEVICE_TYPE_GPU => device_type & d.device_type(),
         CL_DEVICE_TYPE_ALL => CL_DEVICE_TYPE_CUSTOM as u64 & !d.device_type(),
         _ => 0,
      } != 0)
      .collect()
}
