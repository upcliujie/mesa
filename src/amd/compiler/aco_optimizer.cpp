...
label_instruction(opt_ctx& ctx, aco_ptr<Instruction>& instr)
{
   ...
   case aco_opcode::p_extract: {
      if (instr->definitions[0].bytes() == 4) {
         ctx.info[instr->definitions[0].tempId()].set_extract(instr);
         if (instr->operands[0].regClass() == v1 && parse_insert(instr))
   ...
   case aco_opcode::v_mbcnt_lo_u32_b32: {
      if (instr->operands[0].constantEquals(-1) && instr->operands[1].constantEquals(0)) {
         if (ctx.program->wave_size == 32)
            ctx.info[instr->definitions[0].tempId()].set_subgroup_invocation(instr);
         else
            ctx.info[instr->definitions[0].tempId()].set_usedef(instr);
      }
      break;
   ...
   case aco_opcode::v_cvt_f16_f32: {
      if (instr->operands[0].isTemp())
         ctx.info[instr->operands[0].tempId()].set_f2f16(instr);
      break;
   }
