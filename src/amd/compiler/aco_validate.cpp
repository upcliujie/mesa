...
validate_ir(Program* program)
{
   ...
   for (Block& block : program->blocks) {
      for (aco_ptr<Instruction>& instr : block.instructions) {
         ...
         if (get_definition_info(program->gfx_level, instr.get(), 0).bitsize != 0) {
            for (unsigned i = 0; i < 4; i++) {
               SrcDestInfo info = get_definition_info(program->gfx_level, instr.get(), i);
               if (info.bitsize == 0) {
               ...
               if (info.should_be_fixed) {
                  check(instr->definitions[i].isFixed(), "Definition should be fixed.",
                        instr.get());
                  check(instr->definitions[i].physReg() == info.reg,
                        "Definition is fixed to wrong reg.", instr.get());
               }
               if (info.components) {
                  check(instr->definitions[i].size() == info.dwords(), "Definition has wrong size",
                        instr.get());
               } else {
                  assert(info.type == alu_bool);
         ...
         if (get_operand_info(program->gfx_level, instr.get(), 0).bitsize != 0) {
            for (unsigned i = 0; i < 4; i++) {
               SrcDestInfo info = get_operand_info(program->gfx_level, instr.get(), i);
               if (info.bitsize == 0) {
               ...
               if (info.should_be_fixed) {
                  check(instr->operands[i].isFixed(), "Operand should be fixed.", instr.get());
                  check(instr->operands[i].physReg() == info.reg, "Operand is fixed to wrong reg.",
                        instr.get());
               }
               if (info.components) {
                  check(instr->operands[i].size() == info.dwords() ||
