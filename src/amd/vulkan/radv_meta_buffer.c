...
build_buffer_fill_shader(struct radv_device *dev)
{
	...
	nir_store_ssbo(&b, swizzled_load, dst_buf, offset, .mask=0xf, .access=ACCESS_NON_READABLE, .align=16);
...
build_buffer_copy_shader(struct radv_device *dev)
{
	...
	nir_ssa_def *load = nir_load_ssbo(&b, 4, 32, src_buf, offset, .align=16);
	nir_store_ssbo(&b, load, dst_buf, offset, .mask=0xf, .access=ACCESS_NON_READABLE, .align=16);
