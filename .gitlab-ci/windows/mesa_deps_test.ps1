...
$root_mustpass = Join-Path -Path $deqp_source -ChildPath "external\vulkancts\mustpass\master\vk-default\"
foreach($file in @("api", "info", "draw", "query-pool", "memory", "binding-model")) {
  Get-Content "$($root_mustpass)\$($file).txt" | Add-Content -Path "$($deqp_mustpass)\vk-master.txt"
}
...
cargo install --git https://gitlab.freedesktop.org/lfrb/deqp-runner.git --branch windows
