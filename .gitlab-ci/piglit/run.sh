...
# using a command wraper. Hence, we will just set it when running the
# command.
...
if [ "x$PIGLIT_PROFILES" = "xreplay" ] \
       && [ ${PIGLIT_REPLAY_UPLOAD_IMAGES:-0} -eq 1 ]; then

    # We don't want to fail the job if uploading is unsuccessful
    set +e
    ...
    find "$RESULTS/$__PREFIX" -type f -name "*.png" -printf "%P\n" \
        ...
        if grep -q "^$__PREFIX/$__TRACE: pass$" ".gitlab-ci/piglit/$PIGLIT_RESULTS.txt.orig"; then
            ...
            # Let's get the checksum from the YAML file, although it
            # may be faster just to calculate it ? ...
            __CHECKSUM=`PIGLIT_SOURCE_DIR="$CWD" ./bin/replayer.py query -y "$PIGLIT_REPLAY_DESCRIPTION_FILE" checksum -d "$PIGLIT_REPLAY_DEVICE_NAME" "$__TRACE"`
            # __CHECKSUM=`PIGLIT_SOURCE_DIR="$CWD" ./bin/replayer.py checksum "$RESULTS/$__PREFIX/$line"`

            __DESTINATION_FILE_PATH="${__CHECKSUM}.png"
        else
            __MINIO_PATH="$PIGLIT_REPLAY_ARTIFACTS_BASE_URL"
            __DESTINATION_FILE_PATH="$__PREFIX/$line"
        fi
