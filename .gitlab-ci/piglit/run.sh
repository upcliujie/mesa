...
# For non-replay single profile, we will read the tests from file
...
if [ "x$PIGLIT_PROFILES" = "xreplay" ] \
...
elif [ "$PIGLIT_PROFILES" != "${PIGLIT_PROFILES% *}" ]; then
    cp "$INSTALL/piglit/$PIGLIT_RESULTS.txt" \
       ".gitlab-ci/piglit/$PIGLIT_RESULTS.txt.baseline"
else
    ...
    cat $RESULTSFILE | rev | cut -f2- -d: | rev | sed "s/$/:/g" > /tmp/executed.txt

    grep -F -f /tmp/executed.txt "$INSTALL/piglit/$PIGLIT_RESULTS.txt" \
