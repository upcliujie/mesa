...
report_flakes() {
    ...
    echo "Some flakes found (see $1 in artifacts for full results):"
    echo "$flakes" | head -n 50
...
# Report the flakes to the log and to the IRC channel for monitoring (if configured):
quiet report_flakes $RESULTS_CSV

if [ -s $FAILURES_CSV ]; then
    if [ -z "$DEQP_NO_SAVE_RESULTS" ]; then
        echo "Some unexpected results found (see $FAILURES_CSV in artifacts for all failures):"
        head -n 50 $FAILURES_CSV
    else
